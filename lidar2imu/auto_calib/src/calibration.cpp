...
void Calibrator::LoadTimeAndPoes(
...
Eigen::Matrix4d Calibrator::GetDeltaTrans(double R[3], double t[3])
{
    ...
    deltaR << mat[0], mat[3], mat[6], mat[1], mat[4], mat[7], mat[2], mat[5], mat[8];
    ...
    const Eigen::Matrix4d init_Tl2i)
{
    ...
    for (int i = 0; i < frm_start_box.size(); i++)
    {
        ...
        for (size_t frmIdx = 0; frmIdx < frmnum; frmIdx++)
        {
            ...
            std::string lidar_file_name = lidar_path + lidar_files_[real_frmIdx] + ".pcd";
            pcl::PointCloud<LidarPointXYZIRT>::Ptr cloud(new pcl::PointCloud<LidarPointXYZIRT>);
        ...
        std::cout << "delta rpy: " << deltaRPY[0] / degree_2_radian << " "
                  << deltaRPY[1] / degree_2_radian << " " << deltaRPY[2] / degree_2_radian << std::endl;
        std::cout << "delta T: " << deltaT[0] << " " << deltaT[1] << " " << deltaT[2] << std::endl;
    ...
    std::cout << "calib cost " << std::chrono::duration<double>(time_end - time_begin).count() << "s" << std::endl;
    // save refined calib
    ...
    fCalib << "R: " << refined_Ti2l(0, 0) << " " << refined_Ti2l(0, 1) << " " << refined_Ti2l(0, 2) << " "
           << refined_Ti2l(1, 0) << " " << refined_Ti2l(1, 1) << " " << refined_Ti2l(1, 2) << " "
           << refined_Ti2l(2, 0) << " " << refined_Ti2l(2, 1) << " " << refined_Ti2l(2, 2) << std::endl;
    fCalib << "t: " << refined_Ti2l(0, 3) << " " << refined_Ti2l(1, 3) << " " << refined_Ti2l(2, 3) << std::endl;
    ...
    fCalib << bestVal[0] + last_deltaT[0] << " " << bestVal[1] + last_deltaT[1] << " " << bestVal[2] + last_deltaT[2] << " "
           << bestVal[3] + last_deltaT[3] << " " << bestVal[4] + last_deltaT[4] << " " << bestVal[5] + last_deltaT[5] << std::endl;
    std::cout << "save refined calib to " << refine_calib_file << std::endl;
}

void Calibrator::SaveStitching(
    const Eigen::Matrix4d transform,
    const std::string pcd_name)
{
    pcl::PointCloud<pcl::PointXYZI>::Ptr all_cloud(new pcl::PointCloud<pcl::PointXYZI>());
    pcl::octree::OctreePointCloudSearch<pcl::PointXYZI>::Ptr all_octree(new pcl::octree::OctreePointCloudSearch<pcl::PointXYZI>(0.3));
