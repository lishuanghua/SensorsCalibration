...
Registrator::Registrator()
{
    ...
    all_octree_.reset(new pcl::octree::OctreePointCloudSearch<pcl::PointXYZI>(0.1));
...
void Registrator::LoadOdometerData(const std::string odometer_file, const Eigen::Matrix4d &initial_extrinsic)
{
...
void Registrator::LoadLidarPCDs(const std::string &pcds_dir)
{
    ...
    pcl::PointCloud<pcl::PointXYZI>::Ptr g_filter_cloud(new pcl::PointCloud<pcl::PointXYZI>);
    pcl::PointCloud<pcl::PointXYZI>::Ptr ng_filter_cloud(new pcl::PointCloud<pcl::PointXYZI>);
    pcl::PointCloud<pcl::PointXYZI>::Ptr ng_filter_cloud_roi(new pcl::PointCloud<pcl::PointXYZI>);
    PointCloudBbox roi;
    roi.max_x = 100;
    roi.min_x = -100;
    roi.max_y = 50;
    ...
    for (size_t i = 0; i < timestamp_.size(); ++i)
    {
        ...
        printf("\rload: %lu/%lu, %s", i, timestamp_.size() - 1, lidar_file_name.c_str());
    }
    LOGI("load done!");
...
bool Registrator::RegistrationByVoxelOccupancy(Eigen::Matrix4d &transform)
{
    ...
    std::cout << "roll: " << bestVal[0] << ", pitch: " << bestVal[1] << ", yaw: " << bestVal[2]
              << ", tx: " << bestVal[3] << ", ty: " << bestVal[4] << ", tz: " << bestVal[5] << std::endl;
...
bool Registrator::GroundPlaneExtraction(const pcl::PointCloud<pcl::PointXYZI>::Ptr &in_cloud, PlaneParam &plane)
{
    pcl::ModelCoefficients::Ptr coefficients(new pcl::ModelCoefficients);
    pcl::PointIndices::Ptr inliers(new pcl::PointIndices);
    pcl::SACSegmentation<pcl::PointXYZI> seg;
