    ...
    pcl::PointCloud<pcl::PointXYZI>::Ptr pcd_corn)
{
    ...
    // get laser index
    std::vector<std::vector<int>> laser_index(LIDAR_LASER_NUM, std::vector<int>());
    ...
    for (int i = 0; i < LIDAR_LASER_NUM; i++)
    {
        ...
        for (int j = 0; j < cur_point_num; j++)
        {
            ...
            if (j >= 5 && j < cur_point_num - 5)
            {
                ...
                cloudCurvature[real_pidx] = diffX * diffX + diffY * diffY + diffZ * diffZ;
    ...
    for (int i = 0; i < LIDAR_LASER_NUM; i++)
    {
        ...
        for (int j = 0; j < SCAN_LINE_CUT; j++)
        {
            ...
            int ep = scan_start_index[i] + valid_scan_point_num * (j + 1) / SCAN_LINE_CUT - 1;
            ...
            for (int k = ep; k >= sp; k--)
            {
                ...
                std::vector<int>::iterator pos_loc = std::find(laser_index[i].begin() + sp_scan, laser_index[i].begin() + ep_scan, ind);
                ...
                { // 0.1
                    ...
                    for (int l = 1; l <= 5; l++)
                    {
                        float diffX = laserCloud->points[*(pos_loc + l)].x - laserCloud->points[*(pos_loc + l - 1)].x;
                        float diffY = laserCloud->points[*(pos_loc + l)].y - laserCloud->points[*(pos_loc + l - 1)].y;
                        float diffZ = laserCloud->points[*(pos_loc + l)].z - laserCloud->points[*(pos_loc + l - 1)].z;
