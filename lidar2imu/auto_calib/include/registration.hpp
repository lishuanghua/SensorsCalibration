...
public:
    ...
    GroundPlaneExtraction(
        ...
        PlaneParam &plane);
    bool GroundPlaneExtraction(const pcl::PointCloud<pcl::PointXYZI>::Ptr &in_cloud, PlaneParam &g_param);

    void PointCloudDownSampling(const pcl::PointCloud<pcl::PointXYZI>::Ptr &in_cloud,
                                double voxel_size, pcl::PointCloud<pcl::PointXYZI>::Ptr &out_cloud);
    ...
    // load data
    void LoadOdometerData(const std::string odometer_file, const Eigen::Matrix4d &initial_extrinsic);
