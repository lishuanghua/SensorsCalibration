...
{
    BalmVoxelEnergy1(
        const Eigen::Vector3d &current_laser_point,
        const Eigen::Vector3d &point_average,
        const Eigen::Vector3d &normal,
        const Eigen::Matrix4d &imu_tran)
        : current_laser_point_(current_laser_point), point_average_(point_average), normal_(normal), imu_tran_(imu_tran) {}
    ...
    bool operator()(const T *const q, const T *const t, T *residual) const
    {
        ...
        tran_p[0] = imu_tran_(0, 0) * world_p[0] + imu_tran_(0, 1) * world_p[1] +
                    imu_tran_(0, 2) * world_p[2] + imu_tran_(0, 3) - point_average_[0];
        tran_p[1] = imu_tran_(1, 0) * world_p[0] + imu_tran_(1, 1) * world_p[1] +
                    imu_tran_(1, 2) * world_p[2] + imu_tran_(1, 3) - point_average_[1];
        tran_p[2] = imu_tran_(2, 0) * world_p[0] + imu_tran_(2, 1) * world_p[1] +
                    imu_tran_(2, 2) * world_p[2] + imu_tran_(2, 3) - point_average_[2];

        residual[0] = normal_[0] * tran_p[0] + normal_[1] * tran_p[1] + normal_[2] * tran_p[2];
    ...
    static ceres::CostFunction *Create(
        ...
        const Eigen::Matrix4d &imu_tran)
    {
        auto cost_function = new ceres::AutoDiffCostFunction<BalmVoxelEnergy1, 1, 4, 3>(
            new BalmVoxelEnergy1(current_laser_point, point_average, normal, imu_tran));
...
struct BalmVoxelEnergy2
{
    ...
    bool operator()(const T *const q, const T *const t, T *residual) const
    {
        ...
        T imu_point_aver[3];
        T imu_normal[3];
        imu_point_aver[0] = imu_tran_orig_(0, 0) * tran_point_aver[0] + imu_tran_orig_(0, 1) * tran_point_aver[1] +
                            imu_tran_orig_(0, 2) * tran_point_aver[2] + imu_tran_orig_(0, 3);
        imu_point_aver[1] = imu_tran_orig_(1, 0) * tran_point_aver[0] + imu_tran_orig_(1, 1) * tran_point_aver[1] +
                            imu_tran_orig_(1, 2) * tran_point_aver[2] + imu_tran_orig_(1, 3);
        imu_point_aver[2] = imu_tran_orig_(2, 0) * tran_point_aver[0] + imu_tran_orig_(2, 1) * tran_point_aver[1] +
                            imu_tran_orig_(2, 2) * tran_point_aver[2] + imu_tran_orig_(2, 3);

        imu_normal[0] = imu_tran_orig_(0, 0) * tran_normal[0] + imu_tran_orig_(0, 1) * tran_normal[1] +
                        imu_tran_orig_(0, 2) * tran_normal[2];
        imu_normal[1] = imu_tran_orig_(1, 0) * tran_normal[0] + imu_tran_orig_(1, 1) * tran_normal[1] +
                        imu_tran_orig_(1, 2) * tran_normal[2];
        imu_normal[2] = imu_tran_orig_(2, 0) * tran_normal[0] + imu_tran_orig_(2, 1) * tran_normal[1] +
                        imu_tran_orig_(2, 2) * tran_normal[2];

        // n * (p - p_aver)
        tran_p[0] = imu_tran_(0, 0) * world_p[0] + imu_tran_(0, 1) * world_p[1] +
                    imu_tran_(0, 2) * world_p[2] + imu_tran_(0, 3) - imu_point_aver[0];
        tran_p[1] = imu_tran_(1, 0) * world_p[0] + imu_tran_(1, 1) * world_p[1] +
                    imu_tran_(1, 2) * world_p[2] + imu_tran_(1, 3) - imu_point_aver[1];
        tran_p[2] = imu_tran_(2, 0) * world_p[0] + imu_tran_(2, 1) * world_p[1] +
                    imu_tran_(2, 2) * world_p[2] + imu_tran_(2, 3) - imu_point_aver[2];

        residual[0] = imu_normal[0] * tran_p[0] + imu_normal[1] * tran_p[1] + imu_normal[2] * tran_p[2];
    ...
    static ceres::CostFunction *Create(
        ...
        const Eigen::Matrix4d &imu_tran_orig)
    {
        auto cost_function = new ceres::AutoDiffCostFunction<BalmVoxelEnergy2, 1, 4, 3>(
            new BalmVoxelEnergy2(current_laser_point, point_average, normal, imu_tran, imu_tran_orig));
...
struct BalmVoxelEnergy2_NOT
{
    ...
    static ceres::CostFunction *Create(
        ...
        const Eigen::Matrix4d &imu_tran_orig)
    {
        auto cost_function = new ceres::AutoDiffCostFunction<BalmVoxelEnergy2_NOT, 1, 4>(
            new BalmVoxelEnergy2_NOT(current_laser_point, point_average, normal, imu_tran, imu_tran_orig));
...
struct BalmVoxelEnergy3
{
    BalmVoxelEnergy3(
        ...
        : cur_laser_point_(cur_laser_point),
          cur_imu_tran_(cur_imu_tran),
          orig_laser_points_(orig_laser_points),
          ap_centor_origns_(ap_centor_origns),
          imu_trans_(imu_trans) {}
    ...
    bool operator()(const T *const q, const T *const t, T *residual) const
    {
        ...
        residual[0] = overall_normal[0] * tran_p[0] + overall_normal[1] * tran_p[1] + overall_normal[2] * tran_p[2];
    ...
    static ceres::CostFunction *Create(
        ...
        const std::vector<Eigen::Matrix4d> &imu_trans)
    {
        auto cost_function = new ceres::AutoDiffCostFunction<BalmVoxelEnergy3, 1, 4, 3>(
            new BalmVoxelEnergy3(cur_laser_point, cur_imu_tran, orig_laser_points, ap_centor_origns, imu_trans));
...
struct BalmVoxelTransEnergy
{
    BalmVoxelTransEnergy(
        ...
        : current_laser_point_(current_laser_point),
          ...
          imu_tran_(imu_tran),
          imu_tran_orig_(imu_tran_orig),
          refined_calib_mat_(refined_calib_mat) {}
    ...
    static ceres::CostFunction *Create(
        ...
        const Eigen::Matrix4d &refined_calib_mat)
    {
        auto cost_function = new ceres::AutoDiffCostFunction<BalmVoxelTransEnergy, 1, 3>(
            new BalmVoxelTransEnergy(current_laser_point, point_average, normal, imu_tran, imu_tran_orig, refined_calib_mat));
