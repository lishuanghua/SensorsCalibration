...
bool ReadLidarPoses(const std::string &lidar_pose_file, VecMatrixs4d *poses, std::vector<std::string> *pcd_file_names);

void ProcessSingleFrame(const std::string &pcdName, const Eigen::Matrix4d &lidarPose);
...
int main(int argc, char **argv)
{
    ...
    std::cout << "total read " << pcd_file_names.size() << " pcd files with poses\n";
    ...
    while (!pangolin::ShouldQuit())
    {
        ...
        {
            step = cpStep.Get();
            std::cout << "step changed !! current value is: " << step << std::endl;
        }
        // next frame
        ...
        for (size_t i = 0; i < vertexBuffers.size(); i++)
        {
            ...
            glColorPointer(colorBufferCurr->count_per_element, colorBufferCurr->datatype, 0, 0);
            glEnableClientState(GL_COLOR_ARRAY);
            vertexBufferCurr->Bind();
            glVertexPointer(vertexBufferCurr->count_per_element, vertexBufferCurr->datatype, 0, 0);
        ...
        for (int i = 1; i < curFrm; i++)
        {
            ...
            glVertex3d(poses[i - 1](0, 3), poses[i - 1](1, 3), poses[i - 1](2, 3));
            glEnd();
        }
        currIndex = curFrm;
