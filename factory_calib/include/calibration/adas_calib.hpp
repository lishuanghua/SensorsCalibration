...
namespace cameracalib
{
    ...
    public:
         ...
         * @return flag that indicates succeed or failed.
         */
        bool ArucoMarkerBoardCalibration(
            const std::vector<std::vector<float>> &gray_img,
            const InputParam &calibration_inputs,
            const std::string &output_json_path,
            CalibrationOutputParams *calibration_result);
    ...
    public:
        // utils
        bool SaveCalibrationResult(const CalibrationOutputParams &calibration_result, const std::string &output_json_path);

    private:
        bool SaveJsonCalibration(const CalibrationOutputParams &result, const std::string &output_json_file_name);
        bool SaveJsonExtrinsic(const CalibrationOutputParams &result, const std::string &output_json_file_name);
        bool SaveJsonIntrinsic(const CalibrationOutputParams &result, const std::string &output_json_file_name);
        bool LoadJsonIntrinsic(const std::string &input_json_file_name, CameraIntrinsic *camera_intrinsic);
