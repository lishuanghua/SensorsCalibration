...
{
    CameraProjectFactor(const Eigen::Vector3d &object_pt, const Eigen::Vector2d &img_pt, const Eigen::Matrix3d &K)
    ...
    bool operator()(const T *q, const T *t, T *residual) const
    {
        ...
        Eigen::Matrix<T, 3, 1> pt_w{T(object_pt_(0)), T(object_pt_(1)), T(object_pt_(2))};
    ...
    static ceres::CostFunction *Create(
        ...
        const Eigen::Matrix3d &K)
    {
        return (new ceres::AutoDiffCostFunction<CameraProjectFactor, 2, 4, 3>(new CameraProjectFactor(object_pt, img_pt, K)));
    }
...
bool solveCamPnP(
    ...
    std::vector<double> &dist_coeffs, std::vector<float> &rvec,
    std::vector<float> &tvec);
