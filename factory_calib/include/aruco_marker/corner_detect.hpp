...
namespace cameracalib
{
    namespace arucomarker
    {
        ...
        private:
            ...
            std::vector<PointRDP> DepthFirstSearch(std::vector<std::vector<bool>> *img, int i, int j);

            void ApproxPolyDP(const std::vector<PointRDP> src_contour, double eps, std::vector<PointRDP> *rt_contour);

            void findContours(const std::vector<std::vector<bool>> &_in, std::vector<std::vector<PointRDP>> &contours);
            ...
            Eigen::MatrixXf getPerspectiveTransform(std::vector<Point2f> src, std::vector<Point2f> dst);
            ...
            int identifyOneCandidate(
                const std::vector<std::vector<bool>> &gray,
                const std::vector<Point2f> &corners,
                const ArucoMarker &board_pattern)
            {
                return extractBits(gray, corners, board_pattern.marker_size, markerBorderBits_, perspectiveRemovePixelPerCell_,
                                   perspectiveRemoveIgnoredMarginPerCell_, minOtsuStdDev_, board_pattern.id_box);
            }

            bool getPixel(const std::vector<std::vector<bool>> &_src, float org_x, float org_y);

            int findID(std::vector<bool> ID_detect, std::vector<std::vector<bool>> ID_org);
            ...
            std::vector<Point2f> alignContourOrder(Point2f corner, std::vector<Point2f> candidate);
