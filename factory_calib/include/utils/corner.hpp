...
namespace imgproc
{
    static inline void calcMinEigenVal(const std::vector<std::vector<float>> &_cov, std::vector<std::vector<float>> *_dst, int cn)
    {
    ...
    static inline void cornerEigenValsVecs(
        ...
        double k = 0)
    {
        ...
        // three channel
        int channel = 3;
        std::vector<std::vector<float>> cov(height, std::vector<float>(width * channel));

        // img:0-255
        double scale = static_cast<double>(1 << ((aperture_size > 0 ? aperture_size : 3) - 1)) * block_size;
        scale = 1.0 / scale;
    ...
    static inline void cornerMinEigenVal(
    ...
    static inline void minMaxLoc(const std::vector<std::vector<float>> &src, double *maxVal)
    {
        int height = src.size();
        // int width = src[0].size();
        *maxVal = 0;
    ...
    static inline void threshold(std::vector<std::vector<float>> *dst, double thresh, double other)
    {
        int height = dst->size();
    ...
    static inline void binaryThreshold(const std::vector<std::vector<float>> &src, std::vector<std::vector<int>> *dst, int thresh)
    {
        int height = src.size();
        int width = src[0].size();
        int i, j;
