...
namespace cameracalib
{
    ...
    {
        LineConditions() : slopeLimit(true), min_slope(70), max_slope(110), ptsNumLimit(true), min_pts_num(8),
                           max_pts_num(1e5), ptsDistDiffLimit(true), max_pts_dist_diff_percent(5),
                           lineDistDiffLimit(false), max_line_dist_diff(50) {}
    ...
    public:
        ...
        bool Estimate(const std::vector<Eigen::Vector2d> &points, std::vector<std::shared_ptr<LineModel>> *lines);
    ...
    private:
        ...
        double Evaluate(const std::vector<Eigen::Vector2d> &points, double a, double b, double c, std::vector<int> *inliner_idx);

        void GetLineSlope(double x1, double y1, double x2, double y2, double *a, double *b, double *c);

        /*line conditions*/
        bool limitSlope(std::shared_ptr<LineModel> line, double min_slope, double max_slope)
        {
        ...
        bool limitPtsNum(std::shared_ptr<LineModel> line, double min_pts_num, double max_pts_num)
        {
