...
public:
    ...
    void getEndPoint(const std::vector<Eigen::Vector2d> &all_pts, Eigen::Vector2d *endpt1, Eigen::Vector2d *endpt2)
    {
        if (!sorted_)
        {
            this->sort(all_pts);
    ...
    void clusterPoints(const std::vector<Eigen::Vector2d> &all_pts, const double &max_dist_thresh)
    {
        if (!sorted_)
        {
            this->sort(all_pts);
    ...
    void copyFromSortedSegmentLine(std::shared_ptr<LineModel> origin_line, int start_pos, int end_pos)
    {
        ...
        m_b = origin_line->m_b;
        m_c = origin_line->m_c;
        m_inliner_index = std::vector<int>(origin_line->m_inliner_index.begin() + start_pos,
                                           origin_line->m_inliner_index.begin() + end_pos + 1);
        m_line_pts = std::vector<Eigen::Vector2d>(origin_line->m_line_pts.begin() + start_pos,
    ...
    void segment(int start_pos, int end_pos)
    {
        ...
        m_inliner_index = std::vector<int>(old_inliner_index.begin() + start_pos, old_inliner_index.begin() + end_pos + 1);
        m_line_pts = std::vector<Eigen::Vector2d>(old_line_pts.begin() + start_pos, old_line_pts.begin() + end_pos + 1);
...
private:
    ...
    void sort()
    {
        ...
        {
            Eigen::Vector2d pt_proj_vec = projectPointOnLine(m_line_pts[i]) - m_origin_pts[0];
            double dot_num = pt_proj_vec.transpose() * line_vec;
            double directed_dist = pt_proj_vec.norm();
            if (dot_num < 0)
            {
        ...
        std::sort(sort_index.begin(), sort_index.end(), [&pt_origin_dists](size_t i1, size_t i2)
