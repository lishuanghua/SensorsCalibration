...
#define EXTRINSIC_TRANSFORM_METHOD 1
    ...
    static inline void getCam2GroundFromH(
    ...
    static inline void getCam2GroundFromPNP(
        const Eigen::Matrix3d &homography,
        const Eigen::Matrix3d &intrinsic,
        Eigen::Matrix3d *cam2ground)
    {
        ...
        for (size_t i = 0; i < ground_pts_x.size(); i++)
        {
            ...
            {
                ground_pts.emplace_back(Eigen::Vector3d(ground_pts_x[i], ground_pts_y[j], 0));
                Eigen::Vector3d pixel = h_g2cam * Eigen::Vector3d(ground_pts_x[i], ground_pts_y[j], 1);
                pixel /= pixel(2);
