...
private:
    ...
    void GetGridDis(const std::vector<std::vector<Quad>> chess_sets, int *gx_dis, int *gy_dis);

    double CalculateSlope(const std::vector<Quad> &first_chess, const std::vector<Quad> &second_chess);

    double CalculateArea(const std::vector<Quad> &chess);

    bool CheckChessBoardArea(const std::vector<Quad> &first_chess, const std::vector<Quad> &second_chess);

    bool CheckChessboard(
    ...
    std::vector<Point> DepthFirstSearch(std::vector<std::vector<int>> *img, int i, int j);
    ...
    void ApproxPolyDP(const std::vector<Point> src_contour, double eps, std::vector<Point> *rt_contour);
};
