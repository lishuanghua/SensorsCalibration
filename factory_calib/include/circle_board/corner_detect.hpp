...
namespace cameracalib
{
    namespace circleBoard
    {
        ...
        public:
            ...
            bool detect(const std::vector<std::vector<float>> &gray_img, const CircleBoard &board_pattern, CornerPoints *corner_pts);

        private:
            // estimate binary threshold value
            void EstimateGrayThres(const std::vector<std::vector<float>> &img_gray, std::vector<int> *thres_box);

            bool findCirclePattern(const std::vector<Circle> &circles, CornerPoints *corner_pts);
