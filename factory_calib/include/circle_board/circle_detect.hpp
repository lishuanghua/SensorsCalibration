...
namespace cameracalib
{
    namespace circleBoard
    {
        ...
        {
            Circle() {}
            Circle(Point2f mid, float r, std::vector<Point2i> pts) : center(mid), radius(r), points(pts) {}
        ...
        public:
            ...
            bool detect(const std::vector<std::vector<int>> &thresh_img, std::vector<Circle> *circles);

            void generateParam(const int &img_w, const int &img_h);

        private:
            bool FindContours(const std::vector<std::vector<int>> &thresh_vec, std::vector<Contour> *contours);
