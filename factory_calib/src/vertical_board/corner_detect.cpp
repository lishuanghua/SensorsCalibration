...
namespace cameracalib
{
    namespace verticalBoard
    {
            ...
            CornerPoints *detected_corner_points)
        {
            ...
            size_t max_iter = corner_pts.size() * 10;

            for (size_t i = 0; i < thresh_box.size(); ++i)
            {
                double threshold = thresh_box[i]; // pixels
                LineConditions options;
                options.min_pts_num = std::max(static_cast<int>(line_pattern_.lines[1].size() / 2.0 - 1), 7);
                ...
                if (this->selectLinesByLineRelation(
                        line_models, threshold,
            ...
            std::vector<std::vector<int>> *pattern_idxs)
        {
            ...
            for (size_t i1 = 0; i1 < lines.size(); ++i1)
            {
                ...
                if (!this->findRightLeftIdx(left_line_idx, right_line_idx, line_dists, dist_diff_th * 1.5, &left_right_idxs))
                {
                    continue;
                }
                ...
                for (size_t lridx = 0; lridx < left_right_idxs.size(); ++lridx)
                {
                    ...
                    // r_line_idx);
                    double estimated_pt_dist = (line_dists[l_line_idx] + line_dists[r_line_idx]) / 2;
                            ...
                            dist_thresh, &points_line_idx, pattern_idxs, &point_num_score))
                    {
                        ...
                        left_line->copyFromSortedSegmentLine(lines[l_line_idx], start_idxs(0), end_idxs(0));
                        ...
                        right_line->copyFromSortedSegmentLine(lines[r_line_idx], start_idxs(2), end_idxs(2));
            ...
            std::vector<Eigen::Vector2i> *left_right_idxs)
        {
            ...
            for (size_t li = 0; li < left_line_idx.size(); ++li)
            {
                for (size_t ri = 0; ri < right_line_idx.size(); ++ri)
                {
                    ...
                    double dist_diff = fabs(line_dists[l_idx] - line_dists[r_idx]) / std::min(line_dists[l_idx], line_dists[r_idx]);
            ...
            std::vector<std::vector<int>> *pattern_idxs, int *point_num_score)
        {
            ...
            this->projectJointLine(mid_line, left_line, right_line, dist_th * 2, &joint_line, &rough_points_line_idx);

            // first and last corner must be detected after projection
            size_t joint_point_num = joint_line.size();
            // double total_len_diff_th = dist_diff_th/3.0;
            double estimated_total_len = (line_pattern_.lines[1].size() - 1) * estimated_pt_dist;
            ...
            // skip short lines
            if ((joint_line[joint_point_num - 1] - joint_line[0]).norm() < min_total_len_dist)
            {
                return false;
            }
            ...
            // points
            // [start_idx, end_idx]
            size_t max_mid_point = line_pattern_.lines[1].size();
            int max_point_num = 0;
        ...
        {
            int grid_idx = 0;
            size_t line_corner_num = line_pattern_.lines[1].size();
            ...
            size_t min_corner_num = line_pattern_.lines[1].size() / 3;
            for (size_t l = 0; l < 3; ++l)
            {
            ...
            std::vector<Eigen::Vector3i> *points_line_idx)
        {
            ...
            while (pos_idxs[0] < lines[0]->size() || pos_idxs[1] < lines[1]->size() || pos_idxs[2] < lines[2]->size())
            {
                ...
                int min_pos = std::min_element(c_dist.begin(), c_dist.end()) - c_dist.begin();
                int joint_pt_num = 1;
                ...
                for (int i = 0; i < 3; ++i)
                {
                    ...
                    if (fabs(c_dist[i] - min_dist) < dist_th)
                    {
                        ...
                        c_point = (c_point * joint_pt_num + proj_pts[i][pos_idxs[i]]) / static_cast<double>(joint_pt_num + 1);
                        pos_idxs[i]++;
                        joint_pt_num++;
                    }
                }
