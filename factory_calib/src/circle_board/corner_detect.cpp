...
namespace cameracalib
{
    namespace circleBoard
    {
            ...
            CornerPoints *corner_pts)
        {
            ...
            {
                corner_pts->points.resize(board_pattern_.height, std::vector<Point2f>(board_pattern_.width));
        ...
        void CornerDetector::EstimateGrayThres(const std::vector<std::vector<float>> &img_gray, std::vector<int> *thresh_box)
        {
            thresh_box->clear();
            ...
            for (int i = 0; i < img_h; ++i)
            {
                ...
                {
                    aver_gray = (aver_gray * pixel_num + img_gray[i][j]) / static_cast<float>(pixel_num + 1);
            ...
            for (int i = 2; i <= iter; i++)
            {
                ...
                if ((*thresh_box)[i - 1] + stride >= 255 || (*thresh_box)[i - 1] + stride < 0)
                {
        ...
        bool CornerDetector::findCirclePattern(const std::vector<Circle> &circles, CornerPoints *corner_pts)
        {
            ...
            for (auto circle : circles)
            {
                ...
                center_points.emplace_back(Eigen::Vector2d(circle.center.x, circle.center.y));
            }
        ...
        bool CornerDetector::IsBlackCircleLine(
            const std::vector<int> &circle_idx,
            ...
            CornerPoints *corner_pts)
        {
            ...
            for (auto black_pair : possible_black_lines)
            {
                ...
                for (int i = 0; i < line_num; ++i)
                {
                    ...
                    // whether parallel
                    float line_dist = left_line->getLineLineDist(c_line, line_max_angle_diff_);
                    if (line_dist < 0)
                    {
                        continue;
                    }
                ...
                for (size_t bidx = 0; bidx < black_pair.left_blackcircleidx.size(); ++bidx)
                {
                    ...
                    if (this->checkLinePattern(circles, lb_idx, rb_idx, cluster_lines_idxs, line_models, corner_pts))
                    {
                        return true;
                    }
                }
            ...
            std::vector<BlackLines> *possible_black_lines)
        {
            ...
            for (size_t i = 0; i < line_models.size(); ++i)
            {
                ...
                if (this->IsBlackCircleLine(line_models[i]->m_inliner_index, circles, &black_circle_index))
                {
                    black_num++;
                ...
                for (int j = i + 1; j < black_num; ++j)
                {
                    ...
                    if (line_dist > min_line_dist_ && line_dist < max_line_dist_)
                    {
                        ...
                        for (size_t a = 0; a < black_circle_idxs[i].size(); ++a)
                        {
                            ...
                            Eigen::Vector2d a_project = b_line->projectPointOnLine(Eigen::Vector2d(a_center.x, a_center.y));
        ...
        bool CornerDetector::checkLinePattern(
            const std::vector<Circle> &circles,
            const int &left_black_circle_idx,
            const int &right_black_circle_idx,
            const std::vector<int> &line_pos_index,
            const std::vector<std::shared_ptr<LineModel>> &line_models,
            CornerPoints *corner_pts)
        {
            ...
            float estimated_pt_dist = estimated_big_radius * board_pattern_.pt_dist / board_pattern_.big_r;
