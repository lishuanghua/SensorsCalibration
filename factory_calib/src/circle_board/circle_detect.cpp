...
namespace cameracalib
{
    ...
    {
        bool CircleDetector::detect(const std::vector<std::vector<int>> &thresh_img, std::vector<Circle> *circles)
        {
            circles->clear();
            ...
            for (size_t i = 0; i < contours.size(); ++i)
            {
                ...
                if (this->isCircle(contours[i], &circle))
                {
                    ...
                    if (thresh_img[std::round(circle.center.y)][std::round(circle.center.x)] == 0)
                    {
                        circle.black = true;
                    }
        ...
        bool CircleDetector::FindContours(const std::vector<std::vector<int>> &img, std::vector<Contour> *contours)
        {
            contours->clear();
            ...
            for (size_t i = 1; i < height - 1; ++i)
            {
                for (size_t j = 1; j < width - 1; ++j)
                {
                    ...
                    {
                        Contour contour;
                        this->DepthFirstSearch(&img_tmp, &(contour.points), &(contour.center.x), &(contour.center.y), i, j);
        ...
        void CircleDetector::DepthFirstSearch(
