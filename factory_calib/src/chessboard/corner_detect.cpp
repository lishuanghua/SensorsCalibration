...
bool CornerDetect::IsRightTriangle(const std::vector<Quad> &quads)
{
    ...
    return std::fabs(dis1 + dis2 - dis3) < 1000 || std::fabs(dis1 + dis3 - dis2) < 1000 || std::fabs(dis2 + dis3 - dis1) < 1000;
}
...
bool CornerDetect::IsIsoscelesTriangle(const std::vector<Quad> &quads)
{
    ...
    return std::fabs(dis1 - dis2) < 10 || std::fabs(dis1 - dis3) < 10 || std::fabs(dis2 - dis3) < 10;
}
...
void CornerDetect::ClusterByRansac(
    const std::vector<Quad> &cluster_tmp,
...
void CornerDetect::GetGridDis(const std::vector<std::vector<Quad>> chess_sets, int *gx_dis, int *gy_dis)
{
...
double CornerDetect::CalculateSlope(const std::vector<Quad> &first_chess, const std::vector<Quad> &second_chess)
{
    double x1 = (first_chess[0].center.x + first_chess[1].center.x + first_chess[2].center.x) / 3;
    double y1 = (first_chess[0].center.y + first_chess[1].center.y + first_chess[2].center.y) / 3;
    double x2 = (second_chess[0].center.x + second_chess[1].center.x + second_chess[2].center.x) / 3;
    double y2 = (second_chess[0].center.y + second_chess[1].center.y + second_chess[2].center.y) / 3;
...
bool CornerDetect::CheckChessBoardArea(const std::vector<Quad> &first_chess, const std::vector<Quad> &second_chess)
{
    ...
    std::vector<Quad> *valid_Chessboard2)
{
    ...
    if (min_slop < 0.15 && CheckChessBoardArea(*valid_Chessboard1, *valid_Chessboard2))
    {
        return true;
    }
    else
    ...
    float edge_fill_percent)
{
    ...
    {
        for (int j = edge_fill_thickness_y; j < width - edge_fill_thickness_y; j++)
        {
...
void CornerDetect::ImageDilate(
    const std::vector<std::vector<int>> thresh_vec,
    int kernal[3][3],
    ...
    float edge_fill_percent)
{
    ...
    *dilation_vec = thresh_vec;
    ...
    int edge_fill_thickness_x = static_cast<int>(edge_fill_percent * thresh_vec.size() * 1.7);
    int edge_fill_thickness_y = static_cast<int>(edge_fill_percent * thresh_vec[0].size());
    for (size_t i = edge_fill_thickness_x + 1; i < thresh_vec.size() - 1 - edge_fill_thickness_x; i++)
    {
        for (size_t j = edge_fill_thickness_y + 1; j < thresh_vec[i].size() - 1 - edge_fill_thickness_y; j++)
        {
            if (thresh_vec[i][j] != 0)
            {
                for (int k = 0; k < 3; k++)
...
double CornerDetect::CalculateContourArea(std::vector<Point> contour)
{
    ...
    {
        Point2f p = Point2f(static_cast<float>(contour[i].x), static_cast<float>(contour[i].y));
        a00 += static_cast<double>(prev.x * p.y) - static_cast<double>(prev.y * p.x);
        prev = p;
    }
    a00 *= 0.5;
    a00 = std::fabs(a00);
...
std::vector<Point> CornerDetect::DepthFirstSearch(std::vector<std::vector<int>> *img, int i, int j)
{
    ...
    float edge_fill_percent)
{
    ...
    for (size_t i = 1; i < img.size() - 1; i++)
    {
        ...
        {
            if (img[i + 1][j] == 0 && img[i + 1][j - 1] == 0 && img[i][j - 1] == 0 && img[i - 1][j - 1] == 0 &&
                img[i - 1][j] == 0 && img[i - 1][j + 1] == 0 && img[i][j + 1] == 0 && img[i + 1][j + 1] == 0)
            {
                img_tmp[i][j] = 255;
            }
        }
    ...
    std::vector<Point> *grid_center_points)
{
    ...
    for (size_t thres_idx = 0; thres_idx < thres_box.size(); thres_idx += 1)
    {
        ...
        for (size_t i = 0; i < contours.size(); i++)
        {
            ...
            for (int approx_level = 1; approx_level < 8; approx_level++)
            {
                ...
                ApproxPolyDP(approx_contour_tmp, static_cast<double>(approx_level), &approx_contour);
                if (approx_contour.size() == 4)
                {
                    break;
                }
        ...
        double approx_dis_thre = sqrt(area_sum / static_cast<double>(approx_contour_points.size()));
    ...
    endTime = clock();
    std::cout << "Totle Time: " << static_cast<double>(endTime - startTime) / CLOCKS_PER_SEC << std::endl;
