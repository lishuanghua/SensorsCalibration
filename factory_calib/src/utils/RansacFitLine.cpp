...
{
    RansacFitLine::RansacFitLine(LineConditions options, double threshold, int max_iternations)
    {
        options_ = options;
    ...
    bool RansacFitLine::Estimate(const std::vector<Eigen::Vector2d> &points, std::vector<std::shared_ptr<LineModel>> *lines)
    {
        ...
        for (int i = 0; i < max_iternations_; ++i)
        {
            ...
            while (idx2 == idx1 || fabs(points[idx1](0) - points[idx2](0)) + fabs(points[idx1](1) - points[idx2](1)) > thresh_side)
            {
                idx2 = random() % points.size();
            }
            // build the line and find inliners
            GetLineSlope(points[idx1](0), points[idx1](1), points[idx2](0), points[idx2](1),
                         &(line_model->m_a), &(line_model->m_b), &(line_model->m_c));
            ...
            Evaluate(points, line_model->m_a, line_model->m_b, line_model->m_c, &line_model->m_inliner_index);
            ...
            this->clusterLinePoints(points, line_model, options_.max_pts_dist_diff_percent, &clustered_lines);
        ...
        std::vector<int> *inliner_idx)
    {
        ...
        double score = static_cast<double>(inliner_num) / static_cast<double>(total_num);
        ...
        std::vector<std::shared_ptr<LineModel>> *clustered_lines)
    {
        ...
        if (end_pos - start_pos >= 1)
        {
            ...
            new_line->setOriginPts(end_pt1, end_pt2);
            // recompute slope
            this->GetLineSlope(end_pt1(0), end_pt1(1), end_pt2(0), end_pt2(1), &new_line->m_a, &new_line->m_b, &new_line->m_c);
            clustered_lines->emplace_back(new_line);
        }
    }

    // ax + by + c = 0
    void RansacFitLine::GetLineSlope(double x1, double y1, double x2, double y2, double *a, double *b, double *c)
    {
        *a = y2 - y1;
        *b = x1 - x2;
        *c = x2 * y1 - x1 * y2;
