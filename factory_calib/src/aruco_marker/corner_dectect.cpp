...
namespace cameracalib
{
    ...
    {
        bool ArucoMarkerDetector::detect(
            ...
            double minMarkerDistanceRate, bool detectInvertedMarker)
        {
            ...
            for (size_t i = 0; i < candidatesIn.size(); i++)
            {
                ...
                {
                    size_t minimumPerimeter = std::min((int)contoursIn[i].size(), (int)contoursIn[j].size());
            ...
            for (size_t i = 0; i < groupedCandidates.size(); i++)
            {
                ...
                if (biggerIdx > -1)
                {
                    ...
                    {
                        smallerCandidates.emplace_back(alignContourOrder(candidatesIn[biggerIdx][0], candidatesIn[smallerIdx]));
        ...
        std::vector<Point2f> ArucoMarkerDetector::alignContourOrder(Point2f corner, std::vector<Point2f> candidate)
        {
            uint8_t r = 0;
            double min = std::sqrt((corner.x - candidate[0].x) * (corner.x - candidate[0].x) +
                                   (corner.y - candidate[0].y) * (corner.y - candidate[0].y));
            for (uint8_t pos = 1; pos < 4; pos++)
            {
                double nDiff = std::sqrt((corner.x - candidate[pos].x) * (corner.x - candidate[pos].x) +
        ...
        Eigen::MatrixXf ArucoMarkerDetector::getPerspectiveTransform(std::vector<Point2f> src, std::vector<Point2f> dst)
        {
            const int N_inv = 8;
            ...
            float edge_fill_percent)
        {
            ...
            {
                for (size_t j = edge_fill_thickness_y; j < width - edge_fill_thickness_y; j++)
                {
                    aver_gray = (aver_gray * pixel_num + imgGray[i][j]) / static_cast<float>(pixel_num + 1);
            ...
            float edge_fill_percent)
        {
            ...
            for (size_t i = edge_fill_thickness_x; i < dilation_vec.size() - edge_fill_thickness_x; i++)
            {
                for (size_t j = edge_fill_thickness_y; j < dilation_vec[i].size() - edge_fill_thickness_y; j++)
                {
            ...
            int minDistanceToBorder)
        {
            ...
            unsigned int minPerimeterPixels = (unsigned int)(minPerimeterRate * std::max(_in.size(), _in[0].size()));
            unsigned int maxPerimeterPixels = (unsigned int)(maxPerimeterRate * std::max(_in.size(), _in[0].size()));
            ...
            for (size_t i = 0; i < contours.size(); i++)
            {
                ...
                {
                    ApproxPolyDP(contours[i], static_cast<double>(approx_level), &approxCurve);
                ...
                double minDistSq = std::max(_in.size(), _in[0].size()) * std::max(_in.size(), _in[0].size());
                for (int j = 0; j < 4; j++)
                {
                ...
                double minCornerDistancePixels = static_cast<double>(contours[i].size()) * minCornerDistanceRate;
                ...
                if (approxCurve[3].x - approxCurve[1].x == 0 && approxCurve[3].y - approxCurve[1].y == 0)
                {
                    continue;
                }
    ...
    pos = 0
            ...
            while (!s.empty())
            {
                ...
                if (pos != slice.y)
                {
                    ...
                    {
                        READ_PT(pt, pos);
                        double dist = std::fabs((pt.y - start_pt.y) * dx - (pt.x - start_pt.x) * dy);
                        if (dist > max_dist)
                        {
                            max_dist = dist;
            ...
            std::vector<std::vector<PointRDP>> &contours)
        {
            ...
            struct PointRDP **contours_p = trimPoint2dVector(&contour_vector, &contour_size);
            ...
            int dstSize)
        {
            ...
            warp_trans << transformation(0, 0), transformation(1, 0), transformation(2, 0),
                transformation(3, 0), transformation(4, 0), transformation(5, 0),
                transformation(6, 0), transformation(7, 0), transformation(8, 0);
            ...
            for (int i = 0; i < dstSize; i++)
            {
                ...
                {
                    float org_x = (inv_trans(0, 0) * j + inv_trans(0, 1) * i + inv_trans(0, 2)) /
                                  (inv_trans(2, 0) * j + inv_trans(2, 1) * i + inv_trans(2, 2));
                    float org_y = (inv_trans(1, 0) * j + inv_trans(1, 1) * i + inv_trans(1, 2)) /
                                  (inv_trans(2, 0) * j + inv_trans(2, 1) * i + inv_trans(2, 2));
        ...
        bool ArucoMarkerDetector::getPixel(const std::vector<std::vector<bool>> &_src, float org_x, float org_y)
        {
            int low_x = int(org_x);
            int high_x = low_x + 1;
            int low_y = int(org_y);
            int high_y = low_y + 1;
            if (low_x >= 0 && low_y >= 0 && high_x < (int)_src[0].size() && high_y < (int)_src.size())
            {
                double mid_low = (_src[low_y][high_x] - _src[low_y][low_x]) * (org_x - low_x) + _src[low_y][low_x];
                double mid_high = (_src[high_y][high_x] - _src[high_y][low_x]) * (org_x - low_x) + _src[high_y][low_x];
                bool output = static_cast<bool>(std::round((mid_high - mid_low) * (org_y - low_y) + mid_low));
        ...
        int ArucoMarkerDetector::findID(std::vector<bool> ID_detect, std::vector<std::vector<bool>> ID_org)
        {
            int max_cnt = 0;
            int ID = -1;
        ...
        int ArucoMarkerDetector::extractBits(
            ...
            const std::vector<Point2f> &_corners,
            int markerSize,
            ...
            const std::vector<std::vector<bool>> &ID_org)
        {
            ...
            Eigen::MatrixXf transformation = getPerspectiveTransform(_corners, resultImgCorners);
