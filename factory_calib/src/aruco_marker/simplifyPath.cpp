...
simplifyPath::findMaximumDistance(const std::vector<PointRDP> &Points) const
{
    ...
    { // traverse through second PointRDP to second last PointRDP
        PointRDP pp = Points[i] - firstpoint;
        double Dist = fabs(pp * p) / p.Norm(); // formula for PointRDP-to-line distance
        if (Dist > Mdist)
        {
            Mdist = Dist;
            index = i;
...
std::vector<PointRDP> simplifyPath::simplifyWithRDP(std::vector<PointRDP> &Points, double epsilon) const
{
    if (Points.size() < 3)
    { // base case 1
        return Points;
    ...
    if (maxDistance.second >= epsilon)
    {
        ...
        std::vector<PointRDP> path1(Points.begin(), it + index + 1); // new path l1 from 0 to index
        std::vector<PointRDP> path2(it + index, Points.end());       // new path l2 from index to last
