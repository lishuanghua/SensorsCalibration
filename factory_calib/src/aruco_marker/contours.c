...
void initPoint2dVector(struct point2dVector *p2v)
{
    p2v->current_max = 10;
    p2v->current_index = 0;
    p2v->vector = (struct PointRDP **)malloc(sizeof(struct PointRDP *) * (p2v->current_max));
}
...
void initPointVector(struct pointVector *pv)
{
    pv->current_max = 10;
    pv->current_index = 0;
    pv->vector = (struct PointRDP *)malloc(sizeof(struct PointRDP) * (pv->current_max));
}

void resizePoint2dVector(struct point2dVector *p2v)
{
    struct PointRDP **tmp;
    if ((tmp = (struct PointRDP **)realloc(p2v->vector, sizeof(struct PointRDP *) * (p2v->current_max * 2))) == NULL)
    {
        free(p2v->vector);
        perror("malloc failed");
    }
...
{
    struct PointRDP *tmp;
    if ((tmp = (struct PointRDP *)realloc(pv->vector, sizeof(struct PointRDP) * (pv->current_max * 2))) == NULL)
    {
        free(pv->vector);
        perror("malloc failed");
    }
...
void addPoint2dVector(struct point2dVector *p2v, struct PointRDP *point_vector)
{
...
struct PointRDP **trimPoint2dVector(struct point2dVector *p2v, int *vector_size)
{
    struct PointRDP **tmp;
    if ((tmp = (struct PointRDP **)realloc(p2v->vector, sizeof(struct PointRDP *) * (p2v->current_index))) == NULL)
    {
        free(p2v->vector);
        perror("malloc failed");
    }
...
{
    struct PointRDP *tmp;
    if ((tmp = (struct PointRDP *)realloc(pv->vector, sizeof(struct PointRDP) * (pv->current_index))) == NULL)
    {
        free(pv->vector);
        perror("malloc failed");
    }
...
void markExamined(struct PointRDP mark, struct PointRDP center, bool checked[4])
{
    // p3.y, p3.x + 1
    int loc = -1;
    //    3
...
void followBorder(
    int **image, int numrows, int numcols, int y, int x,
    ...
    struct intVector *contour_counter)
{
    ...
    do
    {
        ...
        if (samePoint(current, p2))
        {
            ...
            struct PointRDP *temp = (struct PointRDP *)malloc(sizeof(struct PointRDP));
            temp[0] = start;
    ...
    } while (pixelOutOfBounds(current, numrows, numcols) || image[int(current.y)][int(current.x)] == 0);
    ...
    while (true)
    {
        ...
        if ((p3.x + 1 >= numcols || image[int(p3.y)][int(p3.x) + 1] == 0) && isExamined(checked))
        {
...
void printHierarchy(struct Node *hierarchy, int hierarchy_size)
{
    for (int i = 0; i < hierarchy_size; i++)
    {
        printf("%2d:: parent: %3d first child: %3d next sibling: %3d\n", i + 1,
               hierarchy[i].parent, hierarchy[i].first_child, hierarchy[i].next_sibling);
...
void drawContour(struct PointRDP **contours, int *contour_index, struct Pixel **color, int seq_num, struct Pixel pix)
{
    int r, c;
...
struct Pixel **createChannels(
    int h, int w, struct Node *hierarchy,
...
// save image to bmp
void saveImageFile(
    const char *file_name, int h, int w, struct Node *hierarchy,
    struct PointRDP **contours, int *contour_index,
    int contour_size)
{
    FILE *f;
