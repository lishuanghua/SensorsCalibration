...
namespace cameracalib
{
        ...
        CalibrationOutputParams *calibration_result)
    {
        ...
        base_input_param_.camera_vertical_offset = calibration_inputs.camera_vertical_offset;
        base_input_param_.camera_focus_length_fx = calibration_inputs.camera_focus_length_fx;
        base_input_param_.camera_focus_length_fy = calibration_inputs.camera_focus_length_fy;
        ...
        for (size_t i = 0; i < grid_center_points.size(); i++)
        {
            img_quad.push_back(Point2float(grid_center_points[i].x, grid_center_points[i].y));
        }

        // prepare the corresponding point in the world
        float distance_between_two_chessboard = calibration_inputs.distance_between_two_chessboard;
        ...
        bool homography_flag = CeresSolveHomography(img_quad, world_quad, expected_symmetric_distance, &homo_mat);
        if (!homography_flag)
        {
        ...
        float camera_vertical_pos = calibration_inputs.camera_height - calibration_inputs.board_height;
        Vec3 camera_pos(-calibration_inputs.camera_lateral_offset, camera_vertical_pos, 1);
        ...
        world_quad_ground.push_back(Point2float(car_center_to_board, -pA_chessboard.x));
        world_quad_ground.push_back(Point2float(car_center_to_board, -pB_chessboard.x));

        // prepare midpoints of A-VP and B-VP
        Point2f pA_mid_img((pA(0, 0) + vanishing_point(0, 0)) / 2, (pA(1, 0) + vanishing_point(1, 0)) / 2);
        Point2f pB_mid_img((pB(0, 0) + vanishing_point(0, 0)) / 2, (pB(1, 0) + vanishing_point(1, 0)) / 2);
        ...
        float camera_fy_b = (camera_to_board_distance) * (pB(1, 0) - vanishing_point(1, 0)) / camera_height;
        ...
        pB_mid_ground.x = camera_height * camera_fy / (pB_mid_img.y - vanishing_point(1, 0)) + camera_vertical_offset;
        ...
        homography_flag = CeresSolveHomography(img_quad_ground, world_quad_ground, expected_symmetric_distance, &ground_hmat);
        if (!homography_flag)
        {
            LOGE("failed to compute homography");
        ...
        CalibrationOutputParams *calibration_result)
    {
        ...
        float grid_size = calibration_inputs.chessboard_grid_size;
        float camera_height = calibration_inputs.camera_height;
        size_t mid_corner_num = v_board.lines[1].size();
        float top_board_height = mid_corner_num * grid_size + calibration_inputs.board_height;
        float board_pt_x, board_pt_y;
        for (int i = 0; i < 3; ++i)
        {
            auto c_line = corners.lines_[i];
            size_t pt_num = c_line->m_line_pts.size();
            for (size_t j = 0; j < pt_num; ++j)
            {
                board_pt_x = (i - 1) * grid_size;
                board_pt_y = top_board_height - corners.pattern_idxs_[i][j] * grid_size - camera_height;
        ...
        homography_flag = CeresSolveHomography(ground_img_pts, ground_world_pts, expected_symmetric_distance, &img2ground_hmat);
        if (!homography_flag)
        {
            LOGE("failed to compute homography");
        ...
        CalibrationOutputParams *calibration_result)
    {
        ...
        float camera_vertical_pos = calibration_inputs.camera_height - calibration_inputs.board_height;
        // compute virtual point
        Vec3d camera_pos(-calibration_inputs.camera_lateral_offset, camera_vertical_pos, 1);
        Vec3d vp = img2board_hmat.inverse() * camera_pos;
        vp /= vp(2);
        calibration_result->vanishing_pt(0) = vp(0);
        calibration_result->vanishing_pt(1) = vp(1);
        ...
        CalibrationOutputParams *calibration_result)
    {
        ...
        if (display_img_)
        {
            ...
            for (size_t i = 0; i < corners.points.size(); ++i)
            {
                ...
                cv::putText(display_img, std::to_string(corners.tag_ids[i]), p, cv::FONT_HERSHEY_SIMPLEX, 0.9, cv::Scalar(230, 100, 100));
            }
        ...
        const std::string &output_json_path)
    {
        ...
        std::string intrinsic_json_path = output_dir_ + "front_center_camera-intrinsic.json";
    ...
    bool CalibrationBoardTool::SaveJsonCalibration(const CalibrationOutputParams &result, const std::string &output_json_file_name)
    {
        ...
        front_center_camera_calibration["front_center_camera_calibration"] = Json::Value(data);
        std::cout << "Writing File " << output_json_file_name.c_str() << std::endl;

        Json::StreamWriterBuilder write_builder;
        std::unique_ptr<Json::StreamWriter> json_writer(write_builder.newStreamWriter());
        std::ofstream output;
    ...
    bool CalibrationBoardTool::SaveJsonExtrinsic(const CalibrationOutputParams &result, const std::string &output_json_file_name)
    {
        ...
        front_center_camera_car_center_calib["front_center_camera_car_center_calib"] = Json::Value(data);
        std::cout << "Writing File " << output_json_file_name.c_str() << std::endl;
    ...
    bool CalibrationBoardTool::SaveJsonIntrinsic(const CalibrationOutputParams &result, const std::string &output_json_file_name)
    {
    ...
    bool CalibrationBoardTool::LoadJsonIntrinsic(const std::string &input_json_file_name, CameraIntrinsic *camera_intrinsic)
    {
        ...
        if (Json::parseFromStream(builder, in, &read_data, &errs))
        {
            camera_intrinsic->camera_focus_length_fx =
                read_data["front_center_camera_calib"]["param"]["cam_K"]["data"][0][0].asDouble();
            camera_intrinsic->camera_focus_length_fy =
                read_data["front_center_camera_calib"]["param"]["cam_K"]["data"][1][1].asDouble();
            camera_intrinsic->img_width = read_data["front_center_camera_calib"]["param"]["img_dist_w"].asInt();
            camera_intrinsic->img_height = read_data["front_center_camera_calib"]["param"]["img_dist_h"].asInt();
        }
        else
        {
            return false;
