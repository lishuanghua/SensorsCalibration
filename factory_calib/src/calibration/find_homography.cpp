...
inline double SymmetricGeometricDistance(const Mat3d &h_mat, const Vec2d &x1, const Vec2d &x2)
{
    Vec2d forward_error, backward_error;
    SymmetricGeometricDistanceTerms<double>(h_mat, x1, x2, forward_error.data(), backward_error.data());
...
//
inline bool Homography2DFromCorrespondencesLinearEuc(const MatXd &x1, const MatXd &x2, Mat3d *h_mat, double expected_precision)
{
...
//
template <typename T>
bool HomographySymmetricGeometricCostFunctor::operator()(const T *homography_parameters, T *residuals) const
{
...
ceres::CallbackReturnType TerminationCheckingCallback::operator()(const ceres::IterationSummary &summary)
{
    ...
    Mat3d *h_mat)
{
    ...
    Homography2DFromCorrespondencesLinearEuc(x1, x2, h_mat, EigenDouble::dummy_precision());
    ...
    for (int i = 0; i < x1.cols(); i++)
    {
        ...
        problem.AddResidualBlock(new ceres::AutoDiffCostFunction<HomographySymmetricGeometricCostFunctor,
                                                                 4, // num_residuals
