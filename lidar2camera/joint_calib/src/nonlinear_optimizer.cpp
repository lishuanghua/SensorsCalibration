    ...
    Params &refined_params_)
{
    ...
    v_camera_matrix << camera_matrix(0, 0), camera_matrix(0, 1), camera_matrix(0, 2), camera_matrix(1, 1), camera_matrix(1, 2);
    ...
    for (size_t n = 0; n < params_.vec_rt.size(); ++n)
    {
        ...
        for (int i = 0; i < x1.cols(); i++)
        {
            ReprojectionError *cost_function = new ReprojectionError(x2.col(i), x1.col(i));

            problem.AddResidualBlock(new ceres::AutoDiffCostFunction<ReprojectionError,
                                                                     2, // num_residuals
                                                                     5, 3, 6>(cost_function),
...
void NonlinearOptimizer::formate_data(
    const Eigen::VectorXd &v_camera_matrix_,
    ...
    LidarParams &refined_params_)
{
    ...
    Eigen::VectorXd rot_vec(r.axis() * r.angle());
    Eigen::VectorXd rt(6);
    rt << rot_vec(0), rot_vec(1), rot_vec(2), initial_rt(0, 3), initial_rt(1, 3), initial_rt(2, 3);
    double *p_rt = &rt(0);
    for (size_t i = 0; i < lidar_point_pairs.size(); i++)
    {
        ...
        for (int j = 0; j < 4; j++)
        {
            ...
            LidarReprojectionError *cost_function = new LidarReprojectionError(lidar_2d_point, lidar_3d_point);

            problem.AddResidualBlock(new ceres::AutoDiffCostFunction<LidarReprojectionError,
                                                                     2, // num_residuals
                                                                     5, 2, 6>(cost_function),
                                     NULL, p_camera, p_k, p_rt);
        }
