    ...
    const std::vector<std::vector<std::string>> &lidar_3d_pts)
{
    ...
    for (auto src : lidar_3d_pts)
    {
        ...
        cv::Point3f left_top(std::stod(src[0]), std::stod(src[1]), std::stod(src[2]));
        cv::Point3f right_top(std::stod(src[3]), std::stod(src[4]), std::stod(src[5]));
        cv::Point3f left_bottom(std::stod(src[6]), std::stod(src[7]), std::stod(src[8]));
        cv::Point3f right_bottom(std::stod(src[9]), std::stod(src[10]), std::stod(src[11]));
    ...
    std::vector<cv::Mat> &tvecsMat)
{
    ...
    */
    ...
    for (size_t i = 0; i < rvecsMat.size(); i++)
    {
        ...
        {
            if ((imgpoints_cir[i].x - x_min) <= 50 && (imgpoints_cir[i].y - y_min) <= 50)
            {
        ...
        for (size_t i = 0; i < 4; i++)
        {
            if ((x_max - imgpoints_cir[i].x) <= 50 && (imgpoints_cir[i].y - y_min) <= 50)
            {
                imgpoints_cir1.push_back(imgpoints_cir[i]);
            }
        }
        for (size_t i = 0; i < 4; i++)
        {
            if ((imgpoints_cir[i].x - x_min) <= 50 && (y_max - imgpoints_cir[i].y) <= 50)
            {
                imgpoints_cir1.push_back(imgpoints_cir[i]);
            }
        }
        for (size_t i = 0; i < 4; i++)
        {
            if ((x_max - imgpoints_cir[i].x) <= 50 && (y_max - imgpoints_cir[i].y) <= 50)
            {
                imgpoints_cir1.push_back(imgpoints_cir[i]);
            }
        }
    ...
    for (size_t i = 0; i < _imgs_pts_cir2D_true.size(); i++)
    {
        ...
        std::vector<cv::Point2f> left_top_src, right_top_src, left_bottom_src, right_bottom_src;
    ...
    if (true) // Show the optimized projection effect
    {
        for (size_t i = 0; i < lidar_point_pairs.size(); i++)
        {
            ...
            cv::Point2f img_pt;
            lidar_projection(camera_intrinsic, initial_extrinsic, lidar_point_pairs[i].lidar_3d_point[0], img_pt);
            cv::circle(undistort_img, img_pt, 8, (0, 255, 0), 8);

            cv::Point2f pt = lidar_point_pairs[i].lidar_2d_point[0];
            double error1 = std::sqrt((img_pt.x - pt.x) * (img_pt.x - pt.x) + (img_pt.y - pt.y) * (img_pt.y - pt.y));

            lidar_projection(camera_intrinsic, initial_extrinsic, lidar_point_pairs[i].lidar_3d_point[1], img_pt);
            cv::circle(undistort_img, img_pt, 8, (0, 255, 0), 8);

            pt = lidar_point_pairs[i].lidar_2d_point[1];
            double error2 = std::sqrt((img_pt.x - pt.x) * (img_pt.x - pt.x) + (img_pt.y - pt.y) * (img_pt.y - pt.y));

            lidar_projection(camera_intrinsic, initial_extrinsic, lidar_point_pairs[i].lidar_3d_point[2], img_pt);
            cv::circle(undistort_img, img_pt, 8, (0, 255, 0), 8);

            pt = lidar_point_pairs[i].lidar_2d_point[2];
            double error3 = std::sqrt((img_pt.x - pt.x) * (img_pt.x - pt.x) + (img_pt.y - pt.y) * (img_pt.y - pt.y));

            lidar_projection(camera_intrinsic, initial_extrinsic, lidar_point_pairs[i].lidar_3d_point[3], img_pt);
            cv::circle(undistort_img, img_pt, 8, (0, 255, 0), 8);

            pt = lidar_point_pairs[i].lidar_2d_point[3];
            double error4 = std::sqrt((img_pt.x - pt.x) * (img_pt.x - pt.x) + (img_pt.y - pt.y) * (img_pt.y - pt.y));
            //
    ...
    std::vector<LidarPointPair> &lidar_point_pairs)
{
    ...
    optimier.refine_lidar2camera_params(params, _imgs_pts, _boards_pts, lidar_point_pairs, params_refined);
...
void CameraCalibrator::get_normalization_matrix(const std::vector<cv::Point2f> &pts_, Eigen::Matrix3d &matrix_trans_)
{
