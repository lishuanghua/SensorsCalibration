...
public:
    ...
    void refine_lidar2camera_params(const LidarParams &params_,
                                    const std::vector<std::vector<cv::Point2f>> &imgs_pts_,
                                    const std::vector<std::vector<cv::Point2f>> &bords_pts_,
...
private:
    ...
    {
        HomographySymmetricGeometricCostFunctor(const Eigen::Vector2d &x, const Eigen::Vector2d &y) : x_(x), y_(y) {}
        ...
        bool operator()(const T *homography_parameters, T *residuals) const
        {
            ...
            Utils::SymmetricGeometricDistanceTerms<T>(H, x, y, &residuals[0], &residuals[2]);
            return true;
        }
    ...
    {
        LidarReprojectionError(const Eigen::Vector2d &img_pts_, const Eigen::Vector3d &lidar_pts_)
