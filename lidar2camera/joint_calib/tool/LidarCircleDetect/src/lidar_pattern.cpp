...
void LidarDetector::STLidarDetection(std::string pcds_dir)
{
    ...
    while ((ptr = readdir(dir)) != NULL)
    {
        if (strcmp(ptr->d_name, ".") != 0 && strcmp(ptr->d_name, "..") != 0)
        {
            ...
            {
                std::cout << "Couldn't read file rabbit.pcd" << std::endl;
                exit(1);
            }
            *cloud += *temp_cloude;
        }
    ...
    for (pcl::PointCloud<pcl::PointXYZ>::iterator pt = edges_cloud->points.begin(); pt < edges_cloud->points.end(); ++pt)
    {
    ...
    for (pcl::PointCloud<pcl::PointXYZ>::iterator pt = plane_edges_cloud->points.begin();
    ...
    for (size_t i = 0; i + 240 < pro_map.size(); i += 2)
    {
        for (size_t j = 0; j + 240 < pro_map[0].size(); j += 2)
        {
            int cnt = 0;
            for (size_t ii = i; ii - i <= 240; ii += 2)
            {
                for (size_t jj = j; jj - j <= 240; jj += 2)
                {
                    if (pro_map[ii][jj])
                    {
                        cnt++;
    ...
    for (pcl::PointCloud<pcl::PointXYZ>::iterator pt = initial_centers->points.begin(); pt < initial_centers->points.end(); ++pt)
    {
        ...
        for (int i = initial_y - 20; i <= initial_y + 20; i++)
        {
            for (int j = initial_x - 20; j <= initial_x + 20; j++)
            {
                ...
                {
                    refined_center.y = (refined_center.y * cnt_cnt + float(j) / 200 + min_pt_x) / (cnt_cnt + 1);
                    refined_center.z = (refined_center.z * cnt_cnt + float(i) / 200 + min_pt_y) / (cnt_cnt + 1);
                    cnt_cnt++;
                }
            }
        }
        ...
        std::cout << "pro_ct: " << refined_center.y << ',' << refined_center.z << std::endl;
    ...
    for (pcl::PointCloud<pcl::PointXYZ>::iterator pt = centroid_candidates->points.begin();
