...
Optimizer::Optimizer(const Eigen::Matrix3d *intrinsic, const Eigen::Matrix4d *extrinsic)
{
...
void Optimizer::Calibrate(const cv::Mat *distance_img, const pcl::PointCloud<pcl::PointXYZI>::Ptr register_cloud)
{
    ...
    if (is_violence_search)
    {
                        ...
                        for (int i5 = -5; i5 < 5; i5++)
                        {
                            for (int i6 = -5; i6 < 5; i6++)
                            {
                                ...
                                if (cnt > maxPointCnt)
                                {
                                    ...
                                    std::cout << "match point increase to: " << maxPointCnt << std::endl;
                                }
                            }
                        }
                    }
        ...
        curr_optim_extrinsic_ = curr_optim_extrinsic_ * TransformUtil::GetDeltaT(bestVal);
...
float Optimizer::MaskRegistrationLoss(const float var[6], const cv::Mat *distance_img, const pcl::PointCloud<pcl::PointXYZI>::Ptr register_cloud)
{
    size_t PointCnt = 0;
    ...
    for (const auto &src_pt : register_cloud->points)
    {
        ...
        if (vec_2d(2) > 0)
        {
            ...
            if (x >= 0 && x < distance_img->cols && y >= 0 && y < distance_img->rows)
            {
...
float Optimizer::CostFunction(const float var[6], const cv::Mat *distance_img, const pcl::PointCloud<pcl::PointXYZI>::Ptr register_cloud)
{
    float registration_cost = MaskRegistrationLoss(var, distance_img, register_cloud);

    return registration_cost;
}

void Optimizer::SaveProjectResult(const std::string img_name, const cv::Mat *distance_img, const pcl::PointCloud<pcl::PointXYZI>::Ptr register_cloud)
{
    ...
    Eigen::Vector3d eulerAngle = TransformUtil::Rotation2Eul(rotation_matrix); //.eulerAngles(2, 1, 0);

    std::cout << rotation_matrix << std::endl;
    std::cout << "refine extrinsic: " << std::endl;
    std::cout << "rotation: " << eulerAngle(0) << " " << eulerAngle(1) << " " << eulerAngle(2) << std::endl;
...
void Optimizer::random_search_params(
    int search_count, float delta_x,
    ...
    float delta_yaw)
{
    ...
    std::uniform_real_distribution<double> distribution_roll(-delta_roll, delta_roll);
    std::uniform_real_distribution<double> distribution_pitch(-delta_pitch, delta_pitch);
    std::uniform_real_distribution<double> distribution_yaw(-delta_yaw, delta_yaw);
    float maxPointCnt = CostFunction(var, &register_img_, register_cloud_.makeShared());
