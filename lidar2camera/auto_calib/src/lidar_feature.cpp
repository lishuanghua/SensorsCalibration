    ...
    pcl::PointCloud<pcl::PointXYZI>::Ptr register_cloud)
{
    ...
    pcl::PointCloud<pcl::PointXYZI>::Ptr cloud_car(new pcl::PointCloud<pcl::PointXYZI>);
    pcl::PointCloud<pcl::PointXYZI>::Ptr cloud_filtered(new pcl::PointCloud<pcl::PointXYZI>);
    pcl::PointCloud<pcl::PointXYZI>::Ptr trans_cloud_lane(new pcl::PointCloud<pcl::PointXYZI>);
    int cloud_size = 0;
    if (pcl::io::loadPCDFile<pcl::PointXYZI>(pcd_path_, *cloud) == -1) //* load the file
    {
    ...
    for (const auto &point : *cloud)
    {
        ...
        if (vec_2d(2) > 0)
        {
            ...
            if (x >= 0 && x < distance_images->cols && y >= 0 && y < distance_images->rows)
            {
                if (vec(0) > x_min_threshold_ && vec(0) < x_max_threshold_ && vec(1) > y_min_threshold_ && vec(1) < y_max_threshold_)
                {
                    pcl::PointXYZI pc;
                    pc.x = vec(0);
                    pc.y = vec(1);
    ...
    extract_objects(cloud_filtered, cloud_pole, cloud_lane, cloud_car, trans_cloud_lane);
    pcl::PointCloud<pcl::PointXYZI>::Ptr cloud_lane_pole(new pcl::PointCloud<pcl::PointXYZI>);
    ...
    pcl::PointCloud<pcl::PointXYZI>::Ptr &trans_cloud_lane)
{
    ...
    double alpha = -acos(z_axis.dot(plane_normal) / (z_axis.norm() * plane_normal.norm()));
    ...
    pcl::PointCloud<pcl::PointXYZI>::Ptr trans_ground_point(new pcl::PointCloud<pcl::PointXYZI>);
    pcl::transformPointCloud(*cloud_lane, *trans_ground_point, left_rot_tmp);
    *trans_cloud_lane = *trans_ground_point;

    // lidar height
    pcl::PointCloud<pcl::PointXYZI>::Ptr trans_target_lidar(new pcl::PointCloud<pcl::PointXYZI>);
    pcl::transformPointCloud(*cloud_p, *trans_target_lidar, left_rot_tmp);
    float left_plane_tmp_plane[4] = {0};
    pcl::PointCloud<pcl::PointXYZI>::Ptr lidar_ground_cloud_tmp(new pcl::PointCloud<pcl::PointXYZI>);
    ...
    for (int i = 0; i < ((x_max_threshold_ - x_min_threshold_) / delta_x + 1); i++)
    {
    ...
    {
        if (point.x >= x_min_threshold_ && point.x < x_max_threshold_ && (point.y >= y_min_threshold_ && point.y < y_max_threshold_))
        {
