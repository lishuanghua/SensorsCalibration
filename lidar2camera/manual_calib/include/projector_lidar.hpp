...
public:
        ...
        if (overlap_filter_)
        {
            ...
            for (int32_t m = 0; m < img.rows; m++)
            {
                for (int32_t n = 0; n < img.cols; n++)
                {
                    ...
                    // search window
                    bool front = true;
                    for (int j = std::max(0, m - OVERLAP_FILTER_WINDOW); j < std::min(img.rows, m + OVERLAP_FILTER_WINDOW + 1); j++)
                    {
                        for (int k = std::max(0, n - OVERLAP_FILTER_WINDOW); k < std::min(img.cols, n + OVERLAP_FILTER_WINDOW + 1); k++)
                        {
                            if (filter_pts[j][k] == -1)
                            {
                                continue;
    ...
    cv::Mat ProjectToRawImage(cv::Mat img, Eigen::Matrix3d K, std::vector<double> D, Eigen::Matrix4d json_param)
    {
        cv::Mat K1, D1, R1, T1;
