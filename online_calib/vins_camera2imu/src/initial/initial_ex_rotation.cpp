...
{
    frameT_count++;
    if (angular_velocity_buf.size() > 30 || angular_velocity_buf.size() < 10)
    {
        return false;
    }
...
bool initialExtrinsic::CalibrationExRotation(vector<pair<Vector3d, Vector3d>> corres, Quaterniond delta_q_imu, Matrix3d &calib_ric_result)
{
    ...
    std::cout << "Rimu_ypr: " << Rimu_ypr(0) << ", " << Rimu_ypr(1) << ", " << Rimu_ypr(2) << std::endl;
    std::cout << "Rc_ypr: " << Rc_ypr(0) << ", " << Rc_ypr(1) << ", " << Rc_ypr(2) << std::endl;
    std::cout << "Rc_ideal_ypr: " << Rc_ideal_ypr(0) << ", " << Rc_ideal_ypr(1) << ", " << Rc_ideal_ypr(2) << std::endl;
...
bool initialExtrinsic::CalibrationExPosition(vector<pair<Vector3d, Vector3d>> corres, Quaterniond delta_q_imu, Vector3d delta_p_imu, Matrix3d &calib_ric_result, Vector3d &calib_tic_result)
{
    ...
    if (cflag)
    {
        ...
        Rc.push_back(Matrix3d::Identity());
        Rimu.push_back(ideal_ric.inverse() * ideal_ric);
        tc.push_back(Vector3d::Zero());
        timu.push_back(Vector3d::Zero());
    }
...
double initialExtrinsic::testTriangulation(
...
void initialExtrinsic::decomposeE(
    cv::Mat E,
