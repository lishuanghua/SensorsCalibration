...
namespace autoCalib
{
    namespace calibration
    {
        ...
        bool LidarCarCalibrator::inputLidar(const double &timestamp, PointCloud::Ptr cloud)
        {
            Eigen::Matrix3d car_rot;
            if (!getCarRot(timestamp, car_rot))
            {
                return false;
            }
            ...
            if (frame_cnt_ != 0)
            {
                ...
                LOGI("car rot is: roll = %.3f, pitch = %.3f, yaw = %.3f.", rad2deg(c_rpy(0)), rad2deg(c_rpy(1)), rad2deg(c_rpy(2)));
                LOGI("lidar rot is: roll = %.3f, pitch = %.3f, yaw = %.3f.", rad2deg(l_rpy(0)), rad2deg(l_rpy(1)), rad2deg(l_rpy(2)));
                ...
                // skip static mode
                double move_angle = Eigen::Quaterniond(Eigen::Matrix3d::Identity()).angularDistance(Eigen::Quaterniond(car_rot_prev2now));
                ...
                {
                    if (handeye_calibrator_->calibrate(lidar_pose_rot.block<3, 3>(0, 0), car_rot_prev2now, lidar2car_rot))
                    {
            ...
            Eigen::Vector3d ground_normal = Eigen::Vector3d(vec(0, cov_min_idx), vec(1, cov_min_idx), vec(2, cov_min_idx));
            ...
            LOGI("ground fitting: norm = (%f, %f, %f), intercept = %f",
                 ground_normal(0), ground_normal(1), ground_normal(2), ground_intercept);
            ...
            if (ground_normal(2) < lidar_ground_normal_check_)
            {
                confidence = 0;
            }
            if (ground_intercept < min_intercept_)
            {
                confidence = 0;
            }
            double q_scale = confidence / (calib_conf_ + confidence);
            lidar2car_height_ = q_scale * T_lidar2g(2, 3) + (1 - q_scale) * lidar2car_height_;
            if (calib_conf_ == 0)
            {
                ground_rot_q_ = q_lidar2g;
            }
            else
            {
                ground_rot_q_ = ground_rot_q_.slerp(q_scale, q_lidar2g);
            }
        ...
        bool LidarCarCalibrator::inputLidarT(const double &timestamp, PointCloud::Ptr cloud)
        {
            Eigen::Matrix3d car_rot;
            Eigen::Vector3d car_pos;
            if (!getCarRot(timestamp, car_rot) || !getCarPosition(timestamp, car_pos))
            {
                return false;
            }
        ...
        void LidarCarCalibrator::getTrandformation(Eigen::Matrix4d &lidar2car)
        {
            ...
            lidar2car.block<3, 3>(0, 0) = TransformUtil::GetRotation(ground_rpy(0), (ground_rpy(1) + handeye_rpy(1)) / 2, handeye_rpy(2));
            lidar2car(2, 3) = lidar2car_height_;
        }

        bool LidarCarCalibrator::getCarRot(const double &timestamp, Eigen::Matrix3d &car_rot)
        {
            ...
            double scale = (timestamp - car_timestamp_[idx_min]) / (car_timestamp_[car_pos_idx_] - car_timestamp_[idx_min]);
        ...
        bool LidarCarCalibrator::getCarPosition(const double &timestamp, Eigen::Vector3d &car_pos)
        {
            ...
            double scale = (timestamp - car_timestamp_[idx_min]) / (car_timestamp_[car_pos_idx_1] - car_timestamp_[idx_min]);
        ...
        void LidarCarCalibrator::maxRangeFilter(PointCloud::Ptr cloud_in, PointCloud::Ptr cloud_out)
        {
