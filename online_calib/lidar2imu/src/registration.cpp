...
namespace autoCalib
{
    namespace calibration
    {
        ...
        void Registrator::removeClosedPointCloud(PointCloud::Ptr cloud_in, PointCloud::Ptr cloud_out, float thres)
        {
        ...
        void Registrator::getFeatures(PointCloud::Ptr cloud, PointCloudFeatures &pcd_features)
        {
            ...
            float endOri = -atan2(cloud->points[cloudSize - 1].y, cloud->points[cloudSize - 1].x) + 2 * M_PI;
        ...
        void Registrator::TransformToStart(PointXYZI const *const pi, PointXYZI *const po)
        {
            // interpolation ratio
            double s;
            if (distortion_)
        ...
        void Registrator::TransformToEnd(PointXYZI const *const pi, PointXYZI *const po)
        {
                                     ...
                                     Eigen::Matrix4d &extrinsic)
        {
            ...
            if (0)
            {
                ...
                {
                    TransformToEnd(&input_features.corner_less_sharp_pts->points[i], &input_features.corner_less_sharp_pts->points[i]);
                }

                int surfPointsLessFlatNum = input_features.surf_less_flat_pts->points.size();
                for (int i = 0; i < surfPointsLessFlatNum; i++)
                {
                    TransformToEnd(&input_features.surf_less_flat_pts->points[i], &input_features.surf_less_flat_pts->points[i]);
                }
