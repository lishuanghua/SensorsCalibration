...
namespace autoCalib
{
    namespace calibration
    {
            ...
            const LidarCalibParam &param)
        {
            ...
            std::string gnss2carcenter_path = config_dir + "gnss/gnss-to-car_center-extrinsic.json";
            ...
            for (size_t i = 0; i < lidar_files.size(); i += param.frame_gap)
            {
                ...
                if (l_time - start_timestamp < param.start_sec)
                {
                    continue;
                }
                if (l_time - start_timestamp - param.start_sec > param.max_sec)
                {
                    break;
                }
                // load point cloud
                ...
                if (calibrator_->calib_t)
                {
                    calibrator_->inputLidarT(l_time, lidar_pcd);
                }
                else
                {
                    calibrator_->inputLidar(l_time, lidar_pcd);
                }
            }
        ...
        void LidarToCar::LoadCarPose(
            const std::string &novatel_pos_path,
        ...
        void LidarToCar::LoadPointCloud(const std::string &point_cloud_path, PointCloud::Ptr pcd)
        {
