...
namespace autoCalib
{
    namespace calibration
    {
            ...
            int max_radar_file_num)
        {
            ...
            max_file_num_ = std::min(static_cast<int>(radar_files.size()) - start_file_num_ + 1, max_radar_file_num);
            if (max_file_num_ == 0)
            {
            ...
            {
                if (!getDelphiRadarFrame(radar_file_dir, radar_files, inspva_times, inspva_speed, radar_frames))
                {
                    exit(1);
                }
            }
            ...
            {
                if (!getContiRadarFrame(radar_file_dir, radar_files, inspva_times, inspva_speed, radar_frames))
                {
                    exit(1);
                }
            }
        ...
        bool RadarDataLoader::getNovatelSpeed(const std::string &novatel_inspva, std::vector<double> &timestamps, std::vector<double> &novatel_speed)
        {
            ...
            while (getline(infile, line))
            {
                ...
                if (timestamp < start_timestamp_ - time_gap)
                {
                    continue;
                }
                if (timestamp > end_timestamp_ + time_gap)
                {
                    break;
                }
            ...
            std::vector<RadarFrame> &radar_frames)
        {
            ...
            for (size_t i = start_file_num_ - 1; i < max_file_num_ + start_file_num_ - 1; ++i)
            {
                ...
                while (getline(infile, line))
                {
                    ...
                    if (inspva_speed[inspva_idx] < min_velocity_)
                    {
                        continue;
                    }

                    double speed = this->interpolateSpeed(
                        timestamp, inspva_times[inspva_idx - 1], inspva_times[inspva_idx],
