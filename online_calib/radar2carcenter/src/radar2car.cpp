...
namespace autoCalib
{
    namespace calibration
    {
            ...
            bool savefig)
        {
            ...
            data_loader.getRadarCalibData(radar_file_dir, radar_type, novatel_inspva, radar_datas, param.start_file_num, param.max_file_num);
            ...
            if (radar_type == "conti")
            {
                ...
                for (int i = 0; i < data_num; ++i)
                {
                    ...
                    for (size_t turn = 0; turn < iter; ++turn)
                    {
                        ...
                        double new_yaw;
                        if (!getStationary(start_yaw, data.ve, data.vi, data.track_angle, stationary_idxs))
                        {
                            break;
                        }
                        ...
                        // }
                        if (!fittingCosine(start_yaw, data.ve, data.vi, data.track_angle, stationary_idxs, new_yaw, applyWeight))
                        {
                            break;
                        }
        ...
        bool RadarCalibrator::getRoughYaw(const std::vector<RadarFrame> &radar_datas, const int &frame_gap, double &rough_yaw)
        {
            ...
            for (int i = 0; i < data_num; i += frame_gap)
            {
                int yaw_idx;
                double confidence;
                if (this->getRoughYawSingle(thresh, radar_datas[i].ve, radar_datas[i].vi, radar_datas[i].track_angle, yaw_idx, confidence))
                {
            ...
            std::sort(sort_idx.begin(), sort_idx.end(), [&probability](size_t i1, size_t i2)
                      { return probability[i1] > probability[i2]; });
            ...
            rough_yaw = (max_idx - 18) * rough_deg_gap_ + static_cast<double>(rough_deg_gap_) / 2.0;
            // deg to rad
        ...
        bool RadarCalibrator::getRoughYawSingle(
            const double &thresh,
            ...
            double &confidence)
        {
            ...
            for (size_t i = 0; i < vi.size(); ++i)
            {
                ...
                double similar_c = 1.0 - fabs(ve_scale_ * ve[i] - vi[i]) / ve_scale_ * ve[i];
        ...
        bool RadarCalibrator::getStationary(
            const double &yaw,
            const std::vector<double> &ve,
            const std::vector<double> &vi,
            const std::vector<double> &track_angle,
        ...
        bool RadarCalibrator::fittingCosine(
            const double &yaw,
            const std::vector<double> &ve,
            const std::vector<double> &vi,
            const std::vector<double> &track_angle,
            ...
            bool applyWeight)
        {
            ...
            Eigen::Matrix2d Amat;
            Amat << sin2x, sinxcosx, sinxcosx, cos2x;
            ...
            if (used_frm_ % 10 == 0)
            {
                std::string txtfile = "./fitline/" + std::to_string(pt_num) + "_" + std::to_string(yaw_deg) + ".txt";
        ...
        void RadarCalibrator::saveStationaryTxt(
            const std::vector<double> &track_angle,
        ...
        void RadarCalibrator::saveFitlineTxt(
