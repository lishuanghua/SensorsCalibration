...
public:
    ...
    static Eigen::Matrix3d eulerAnglesToRotationMatrix(Vec3f &theta) // theta为角度制
    {
        ...
        Mat R_x = (Mat_<double>(3, 3) << 1, 0, 0, 0, cos(th0), -sin(th0), 0, sin(th0), cos(th0));

        // Calculate rotation about y axis
        Mat R_y = (Mat_<double>(3, 3) << cos(th1), 0, sin(th1), 0, 1, 0, -sin(th1), 0, cos(th1));

        // Calculate rotation about z axis
        Mat R_z = (Mat_<double>(3, 3) << cos(th2), -sin(th2), 0, sin(th2), cos(th2), 0, 0, 0, 1);
