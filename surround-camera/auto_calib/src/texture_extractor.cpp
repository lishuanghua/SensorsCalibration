...
extractor::extractor(
    Mat img1_bev_, Mat img2_bev_, Mat intrinsic1_, Mat intrinsic2_,
    Eigen::Matrix4d extrinsic1_, Eigen::Matrix4d extrinsic2_)
{
    img1_bev = img1_bev_;
    img2_bev = img2_bev_;
...
void extractor::Binarization()
{
    ...
    for (int i = 0; i < rows; i++)
    {
        ...
        {
            if (img1_bev.at<Vec3b>(i, j) != Vec3b(0, 0, 0) && img2_bev.at<Vec3b>(i, j) != Vec3b(0, 0, 0))
            {
...
void extractor::findcontours()
{
    ...
    // getStructuringElement()函数的作用是返回一个结构元素（卷积核）
    // getStructuringElement函数会返回指定形状和尺寸的结构元素。
    Mat dilate_kernel = getStructuringElement(0, Size(5, 5));
    // cv::dilate 通过一个特定的结构元素膨胀一个图像。
...
std::vector<std::vector<cv::Point>> extractor::fillContour(const std::vector<std::vector<cv::Point>> &_contours)
{
...
vector<cv::Point> extractor::extrac_textures_and_save(string pic_filename, string csv_filename, string idx, double size)
{
    int down_sample = 500;
    Mat gray1, gray2;
    ...
    if (exposure_flag)
    {
        // cv::mean该函数会得到Mat中各个通道的均值，若要获取指定通道的均值，做进一步解析即可。
        ncoef = mean(gray1).val[0] / mean(gray2).val[0];
    }
    // cv::GaussianBlur()函数是OpenCV中用于对图像进行高斯模糊的函数。
    ...
    for (auto pixel : contour1_pixels)
    {
        ...
        if (edge_flag && (abs(pixel.x - camera_center_x) > (size * 4.5 / 5) || abs(pixel.y - camera_center_y) > (size * 4.5 / 5)))
        {
            continue;
        }
        Eigen::Vector2d delta((gray1.ptr<uchar>(pixel.y)[pixel.x] - gray1.ptr<uchar>(pixel.y)[pixel.x - 2]),
