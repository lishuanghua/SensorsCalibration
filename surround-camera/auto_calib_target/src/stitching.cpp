...
void initializeK(Eigen::Matrix3d &K_F, Eigen::Matrix3d &K_L, Eigen::Matrix3d &K_B, Eigen::Matrix3d &K_R)
{
...
void initializeD(Eigen::Vector4d &D_F, Eigen::Vector4d &D_L, Eigen::Vector4d &D_B, Eigen::Vector4d &D_R)
{
    D_F << 0, 0, 0, 0;
    D_L << 0, 0, 0, 0;
    D_B << 0, 0, 0, 0;
...
void initializeExtrinsic(Eigen::Matrix4d &T_FG, Eigen::Matrix4d &T_LG, Eigen::Matrix4d &T_BG, Eigen::Matrix4d &T_RG)
{
    Eigen::Matrix4d T_CG;
...
cv::Mat project_on_ground(
    cv::Mat img, Eigen::Matrix4d T_CG, Eigen::Matrix3d K_C,
    Eigen::Vector4d D_C, cv::Mat K_G, int rows, int cols)
{
    ...
    // cv::split 函数是一个图像分割函数，它可以将一幅图像分割成多个通道。
    ...
    // reshape函数是一个非常有用的函数，在计算机视觉中经常被用于改变数组的形状和合并多个数组。
    ...
    cv::Mat img_GC;
    // cv::remap,简单点说就是把输入图像中各个像素按照一定的规则映射到另外一张图像的对应位置上去，形成一张新的图像。
    cv::remap(img, img_GC, p_GC_table_32F, cv::Mat(), cv::INTER_LINEAR);

    return img_GC;
}

void generate_surround_view(
    cv::Mat surround_view, cv::Mat img_F, cv::Mat img_L, cv::Mat img_B, cv::Mat img_R,
    ...
    Eigen::Matrix3d K_F, Eigen::Matrix3d K_L, Eigen::Matrix3d K_B, Eigen::Matrix3d K_R)
{
    ...
    // cv::addWeighted 是OpenCV库中的一个函数，用于执行两个图像的加权和。
