...
{
    K_F = (Mat_<double>(3, 3) << 390.425287, 0, 750, 0, 390.425287, 750, 0, 0, 1);
    K_L = (Mat_<double>(3, 3) << 390.425287, 0, 750, 0, 390.425287, 750, 0, 0, 1);
    K_B = (Mat_<double>(3, 3) << 390.425287, 0, 750, 0, 390.425287, 750, 0, 0, 1);
    K_R = (Mat_<double>(3, 3) << 390.425287, 0, 750, 0, 390.425287, 750, 0, 0, 1);
...
int main(int argc, char **argv)
{
    ...
    // N点透视位姿求解（Perspective-N-Point，PNP问题）
    solvePnP(pts_3d_L, pts_2d_L, K_L, D_L, rvecs_L, translation_L);
    // cv::solvePnP()解算得出的是旋转向量，而为了求得以欧拉角形式表达的位姿关系，需要转换为旋转矩阵。
    // 旋转向量与旋转矩阵之间的变换又称为罗德里格斯变换（Rodrigues）。
    Rodrigues(rvecs_L, rotation_L);
    // 对给定的矩阵应用进行水平连接。函数垂直连接两个或更多的cv::Mat矩阵 (具有相同的行数)。
