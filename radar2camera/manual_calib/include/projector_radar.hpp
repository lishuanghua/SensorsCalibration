...
public:
    ...
    void init(const int &width, const int &height, const Eigen::Matrix3d &img2ground_hmat)
    {
        width_ = width;
        height_ = height;
        ...
        for (int i = 0; i < 4; i++)
        {
            Eigen::Vector3d img_pt = ground2img_hmat_ * Eigen::Vector3d(ground_pts[i].x, ground_pts[i].y, 1);
            img_pt /= img_pt(2);
    ...
    cv::Mat drawProjectBirdView(const cv::Mat &img, const int &point_size, const std::vector<cv::Point2f> grount_pts)
    {
        cv::Mat output;
...
public:
    ...
    void init(const cv::Mat &img, const Eigen::Matrix3d &homograph, const int &bv_w, const int &bv_h)
    {
    ...
    cv::Mat ProjectToFisheyeMat(cv::Mat img, cv::Mat K, cv::Mat D, cv::Mat R, cv::Mat T)
    {
        cv::Mat I = cv::Mat::eye(3, 3, CV_32FC1);
        cv::Mat mapX, mapY;
        cv::Mat outImg = cv::Mat(img.size(), CV_32FC3);
    ...
    cv::Mat ProjectToFisheyeImage(std::string imgName, cv::Mat K, cv::Mat D, cv::Mat R, cv::Mat T)
    {
        cv::Mat img = cv::imread(imgName);
