...
public:
    ...
    bool computeHomo()
    {
        ...
        if (picked_num != 4)
        {
            std::cerr << "[ERROR]Invalid Labeled Point Num, Must select 4 points(leftdown, leftup, rightdown, rightup).\n";
        ...
        for (int i = 0; i < 4; i++)
        {
            Eigen::Vector3d ground_pt = img2ground_hmat_ * Eigen::Vector3d(picked_points_[i].x, picked_points_[i].y, 1);
        ...
        grid_image_ = cv::Mat(cv::Size(width_, height_), CV_32FC3, cv::Scalar(235, 235, 235));
        // draw lines
        ...
        // draw lane lines
        cv::line(grid_image_, cv::Point(int(left), 0), cv::Point(int(left), height_), cv::Scalar(225, 80, 0), 4);
        cv::line(grid_image_, cv::Point(int(right), 0), cv::Point(int(right), height_), cv::Scalar(225, 80, 0), 4);
        cv::imwrite("grid_image.jpg", grid_image_);

        return true;
    }

    void init(const cv::Mat &img, const int &width, const int &height, const Eigen::Matrix3d &img2ground_hmat)
    {
        origin_img_size_ = img.size();
        img.copyTo(image_);
        cv::putText(image_, "Pick 4 lane points. Press Enter to compute homography.",
                    cv::Point(50, 50), cv::FONT_HERSHEY_SIMPLEX, 1.3, cv::Scalar(30, 30, 30), 4);
    ...
    cv::Mat drawProjectBirdView(const int &point_size, const std::vector<cv::Point2f> &grount_pts)
    {
        cv::Mat output;
