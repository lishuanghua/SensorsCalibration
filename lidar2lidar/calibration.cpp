    ...
    const std::map<int32_t, InitialExtrinsic> extrinsics)
{
    ...
    for (auto src : extrinsics)
    {
        ...
        Eigen::Matrix4d init_ext = TransformUtil::GetMatrix(extrinsic.t_matrix, rot);
...
void Calibrator::Calibrate()
{
    ...
    for (auto iter = pcs_.begin(); iter != pcs_.end(); iter++)
    {
        ...
        // 函数的作用是通过转换矩阵init_ext将slave_pc转换后存到trans_cloud中保存。
    ...
    for (auto iter = pcs_.begin(); iter != pcs_.end(); iter++)
    {
        ...
        Eigen::Matrix4d init_ext = init_extrinsics_[slave_id];
        LOGI("init extrinsic T_ms is: roll = %f, pitch = %f, yaw =  %f, x = %f, y = %f, z = %f\n",
             TransformUtil::GetRoll(init_ext) / degree_2_radian,
             TransformUtil::GetPitch(init_ext) / degree_2_radian,
             TransformUtil::GetYaw(init_ext) / degree_2_radian, init_ext(0, 3), init_ext(1, 3), init_ext(2, 3));
        ...
        if (slave_id)
        {
            ...
            range_condition->addComparison(pcl::FieldComparison<pcl::PointXYZI>::ConstPtr(new pcl::FieldComparison<pcl::PointXYZI>("x", pcl::ComparisonOps::GT, -1)));   // GT表示大于等于
            range_condition->addComparison(pcl::FieldComparison<pcl::PointXYZI>::ConstPtr(new pcl::FieldComparison<pcl::PointXYZI>("x", pcl::ComparisonOps::LT, 1)));    // LT表示小于等于
            range_condition->addComparison(pcl::FieldComparison<pcl::PointXYZI>::ConstPtr(new pcl::FieldComparison<pcl::PointXYZI>("y", pcl::ComparisonOps::GT, -1.0))); // GT表示大于等于
            range_condition->addComparison(pcl::FieldComparison<pcl::PointXYZI>::ConstPtr(new pcl::FieldComparison<pcl::PointXYZI>("y", pcl::ComparisonOps::LT, 1)));    // LT表示小于等于
            range_condition->addComparison(pcl::FieldComparison<pcl::PointXYZI>::ConstPtr(new pcl::FieldComparison<pcl::PointXYZI>("z", pcl::ComparisonOps::GT, -1)));   // GT表示大于等于
            range_condition->addComparison(pcl::FieldComparison<pcl::PointXYZI>::ConstPtr(new pcl::FieldComparison<pcl::PointXYZI>("z", pcl::ComparisonOps::LT, 1)));    // LT表示小于等于
        ...
        Eigen::Matrix4d init_guess = TransformUtil::GetMatrix(init_x, init_y, z, roll, pitch, init_yaw);
