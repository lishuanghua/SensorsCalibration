                                 ...
                                 const int &board_width, const int &board_height)
{
  ...
  area_box_ = std::vector<std::vector<int>>(img_height_, std::vector<int>(img_width_, 5));
...
bool AutoImagePicker::checkMoveThresh(const BoardSquare &board)
{
  for (size_t i = 0; i < candidate_board_.size(); i++)
  {
    ...
    if (dist < CHESSBOARD_MIN_MOVE_THRESH * double(img_width_ + img_height_) / 2.0)
    {
      return false;
    }
  }
...
{
  int score = 0;
  for (int i = std::max(0, int(board.min_y)); i < std::min(img_height_, int(board.max_y)); i++)
  {
    for (int j = std::max(0, int(board.min_x)); j < std::min(img_width_, int(board.max_x)); j++)
    {
      score += area_box_[i][j];
    }
  }

  if (score < CHESSBOARD_MIN_AREA_CHANGE_PARTION_THRESH * img_width_ * img_height_)
  {
    return false;
  }
  else
...
bool AutoImagePicker::checkPoseAngleThresh(const BoardSquare &board)
{
  for (size_t i = 0; i < candidate_board_.size(); i++)
  {
    ...
    if (dist < CHESSBOARD_MIN_MOVE_THRESH * double(img_width_ + img_height_) / 2.0)
    {
      if (fabs(board.angle_left_top - candidate.angle_left_top) < CHESSBOARD_MIN_ANGLE_CHANGE_THRESH &&
          fabs(board.angle_right_top - candidate.angle_right_top) < CHESSBOARD_MIN_ANGLE_CHANGE_THRESH &&
