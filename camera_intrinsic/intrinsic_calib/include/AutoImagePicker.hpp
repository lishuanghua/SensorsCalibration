...
public:
                ...
                const cv::Point &p4, const int &idx = 0)
    {
        ...
        angle_left_top = VectorAngle(p2.x - p1.x, p2.y - p1.y, p3.x - p1.x, p3.y - p1.y);
        angle_right_top = VectorAngle(p1.x - p2.x, p1.y - p2.y, p4.x - p2.x, p4.y - p2.y);
        angle_left_bottom = VectorAngle(p1.x - p3.x, p1.y - p3.y, p4.x - p3.x, p4.y - p3.y);
        angle_right_bottom = 360.0 - angle_left_top - angle_right_top - angle_left_bottom;
        ...
        double d13 = sqrt((p3.x - p1.x) * (p3.x - p1.x) + (p3.y - p1.y) * (p3.y - p1.y));
        double d24 = sqrt((p2.x - p4.x) * (p2.x - p4.x) + (p2.y - p4.y) * (p2.y - p4.y));
        double d34 = sqrt((p3.x - p4.x) * (p3.x - p4.x) + (p3.y - p4.y) * (p3.y - p4.y));
        area = 1 / 2.0 * sin(angle_left_top * M_PI / 180.0) * d12 * d13 +
               1 / 2.0 * sin(angle_right_bottom * M_PI / 180.0) * d24 * d34;
        index = idx;
    }

    double VectorAngle(const double &x1, const double &y1, const double &x2, const double &y2)
    {
        double f = (x1 * x2 + y1 * y2) / sqrt(x1 * x1 + y1 * y1) / sqrt(x2 * x2 + y2 * y2);
...
private:
    ...
    std::vector<std::vector<BoardSquare>> board_square_box_; // divided into left, middle, right 3 areas
};