...
bool GroundExtractor::LPRFitting(const PointCloudPtr in_cloud, PointCloudPtr g_cloud, PointCloudPtr ng_cloud, PlaneParam *plane)
{
    // find the lpr plane
    PointCloudPtr sort_cloud(new PointCloud());
    PCUtil::MaxRangeFilter(in_cloud, 50.0, sort_cloud);
    std::sort(sort_cloud->points.begin(), sort_cloud->points.end(),
    ...
    size_t lpr_num = static_cast<size_t>(lpr_least_gpoints_rate_ * sort_cloud->points.size());
    ...
    for (; iter_cnt < lpr_max_iters_; iter_cnt++)
    {
        ...
        if (dlt_norm.norm() < 0.001 && dlt_intcpt < 0.01 && iter_cnt > lpr_max_iters_ / 2)
        {
            break;
        }
...
bool GroundExtractor::RandomRansacFitting(
    const PointCloudPtr in_cloud,
    ...
    PlaneParam *plane)
{
    ...
    for (int i = 0; i < rr_iter_times_; i++)
    {
        for (int j = 0; j < rr_max_rand_iters_; j++)
        {
            ...
            {
                PointType point = in_cloud->points[j];
                double point_to_plane_dis = std::fabs(pp.normal(0) * point.x + pp.normal(1) * point.y +
    ...
    {
        PointType point = in_cloud->points[j];
        double point_to_plane_dis = std::fabs(best_plane.normal(0) * point.x + best_plane.normal(1) * point.y +
...
bool GroundExtractor::RandomSearchPlane(
    const PointCloudPtr in_cloud,
    PlaneParam &best_plane,
    ...
    int iteration_times)
{
    ...
    for (int i = 0; i < iteration_times; i++)
    {
        ...
        {
            PointType point = in_cloud->points[j];
            double point_to_plane_dis = std::fabs(tmp_plane.normal(0) * point.x + tmp_plane.normal(1) * point.y +
...
bool GroundExtractor::CalArea(const PointType &p1, const PointType &p2, const PointType &p3, double *area)
{
...
bool GroundExtractor::FittingPlane(PointCloudPtr in_cloud, PlaneParam *plane)
{
    Eigen::Vector3d point(in_cloud->points[0].x, in_cloud->points[0].y, in_cloud->points[0].z);
    Eigen::Vector3d vec1(point(0) - in_cloud->points[1].x, point(1) - in_cloud->points[1].y, point(2) - in_cloud->points[1].z);
    Eigen::Vector3d vec2(point(0) - in_cloud->points[2].x, point(1) - in_cloud->points[2].y, point(2) - in_cloud->points[2].z);
...
bool GroundExtractor::FittingPlaneMesh(const PointCloudPtr in_cloud, PlaneParam *plane)
{
    ...
    // svd
    Eigen::JacobiSVD<Eigen::MatrixXd> svd(cov, Eigen::DecompositionOptions::ComputeFullU);
