...
public:
    ...
    bool LPRFitting(const PointCloudPtr in_cloud, PointCloudPtr g_cloud, PointCloudPtr ng_cloud, PlaneParam *plane);

    /* @brief: ground extraction using random ransac algorithm,
     * aim at the slaver lidar*/
    bool RandomRansacFitting(const PointCloudPtr in_cloud, PointCloudPtr g_cloud, PointCloudPtr ng_cloud, PlaneParam *plane);

private:
    size_t RandIndex(size_t range);

    bool CalArea(const PointType &p1, const PointType &p2, const PointType &p3, double *area);
