...
namespace autoCalib
{
    namespace calibration
    {
            ...
            bool savefig)
        {
            ...
            data_loader.getRadarCalibData(radar_file_dir, radar_type, novatel_enu, radar_datas, param.start_sec, param.max_sec);
        ...
        bool RadarCalibrator::whetherStatic(const RadarObject &object)
        {
            ...
            for (int i = min_gap_; i < object_size; i++)
            {
                ...
                double angle_diff = std::min(fabs(phi0 + gama - phi1), fabs(phi0 - gama - phi1));
                if (angle_diff > angle_thresh_)
                {
                    return false;
                }
        ...
        bool RadarCalibrator::getSingleYaw(const RadarObject &object, int i0, int i1, double &yaw_deg, double &confidence)
        {
