...
namespace autoCalib
{
    ...
    {
        void RadarDataLoader::preLoadImu(
            ...
            double max_sec)
        {
            ...
            while (getline(enu_file, line))
            {
                ...
                ImuFrame imu_frm;
                if (!timeString2timecount(elements[0], imu_frm.timestamp))
                {
                    continue;
                }
                imu_frm.yaw = stod(elements[10]) * RAD2DEG;
                imu_frm.v = Eigen::Vector3d(stod(elements[6]), stod(elements[5]), stod(elements[7])).norm();
        ...
        bool RadarDataLoader::getStraightSegment(const std::vector<ImuFrame> &imu_frames, std::vector<std::vector<int>> &segment_idxes)
        {
            segment_idxes.clear();
            int i = 0;
            ...
            double max_sec)
        {
            ...
            {
                if (!getDelphiRadarFrame(radar_file_dir, radar_files, imu_frames, segment_idxs, radar_frames))
                {
                    exit(1);
                }
            }
            ...
            {
                if (!getContiRadarFrame(radar_file_dir, radar_files, imu_frames, segment_idxs, radar_frames))
                {
                    exit(1);
                }
            }
        ...
        bool RadarDataLoader::getDelphiRadarFrame(
            const std::string &radar_dir,
            ...
            std::vector<RadarObject> &radar_objects)
        {
            ...
            for (size_t i = 0; i < radar_files.size(); ++i)
            {
                ...
                if (file_time + DELPHI_TIMEGAP < start_time)
                {
                    continue;
                }
                ...
                while (getline(infile, line))
                {
                    ...
                    if (timestamp >= start_time && timestamp < end_time)
                    {
                        ...
                        // imu data
                        double speed = this->interpolate(timestamp, imu_frames[imu_idx - 1].timestamp, imu_frames[imu_idx].timestamp,
                                                         imu_frames[imu_idx - 1].v, imu_frames[imu_idx].v);
                        double x = this->interpolate(timestamp, imu_frames[imu_idx - 1].timestamp, imu_frames[imu_idx].timestamp,
                                                     imu_frames[imu_idx - 1].x, imu_frames[imu_idx].x);
                        double y = this->interpolate(timestamp, imu_frames[imu_idx - 1].timestamp, imu_frames[imu_idx].timestamp,
                                                     imu_frames[imu_idx - 1].y, imu_frames[imu_idx].y);
                        double z = this->interpolate(timestamp, imu_frames[imu_idx - 1].timestamp, imu_frames[imu_idx].timestamp,
        ...
        bool RadarDataLoader::getContiRadarFrame(
            const std::string &radar_dir,
            const std::vector<std::string> &radar_files,
            const std::vector<ImuFrame> &imu_frames,
            const std::vector<std::vector<int>> &segment_idxs,
